...
func TestHandleProtocolsList_Happy(t *testing.T) {
	...
	d := env.Data
	if d.TotalProtocols != 18 || len(d.Protocols) != 18 {
		t.Fatalf("total = %d (len %d), want 18", d.TotalProtocols, len(d.Protocols))
	}
...
func TestHandleProtocols_NilReaderDegradation(t *testing.T) {
	...
	if env.Data.TotalProtocols != 18 {
		t.Fatalf("total = %d, want 18", env.Data.TotalProtocols)
	}
	for _, p := range env.Data.Protocols {
