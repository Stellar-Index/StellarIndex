...
func (s *Server) handleChart(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if handlerTimedOut(chartCtx, err) {
			...
			writeProblem(w, r,
				"https://api.stellaratlas.xyz/errors/chart-timeout",
...
func parseChartParams(w http.ResponseWriter, r *http.Request) (string, chartTimeframeSpec, string, string, bool) {
	...
	if !ok {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/invalid-timeframe",
	...
	if priceType == "twap" {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/price-type-not-supported",
	...
	if priceType != "vwap" {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/invalid-price-type",
...
func (s *Server) handleChartMarketCap(
	...
	if pair.Quote.Type != canonical.AssetFiat || pair.Quote.Code != "USD" {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/invalid-market-cap-quote",
	...
	if pair.Base.Type != canonical.AssetFiat {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/market-cap-deferred",
	...
	if s.verifiedCurrencies == nil || s.fxHistory == nil {
		writeProblem(w, r,
			"https://api.stellaratlas.xyz/errors/market-cap-unavailable",
