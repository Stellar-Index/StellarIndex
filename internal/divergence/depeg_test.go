...
func TestStablecoinDepeg_DivergenceWorkerFires(t *testing.T) {
	...
	body, err := rdb.Get(context.Background(), cachekeys.Divergence(xlm)).Bytes()
	if err != nil {
		t.Fatalf("redis get %s: %v", cachekeys.Divergence(xlm), err)
	}
...
func TestStablecoinPegHolds_DivergenceWorkerStaysQuiet(t *testing.T) {
	...
	body, _ := rdb.Get(context.Background(), cachekeys.Divergence(xlm)).Bytes()
	var cached divergence.CachedResult
