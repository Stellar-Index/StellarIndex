...
func parseRegionList(csv string) ([]regionEndpoint, error) {
	out := []regionEndpoint{}
	for _, kv := range opsutil.SplitCSV(csv) {
...
func fetchOneRegion(
	...
	if httpResp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("status %d, body=%s", httpResp.StatusCode, opsutil.Truncate(string(body), 200))
	}
	...
	if err := json.Unmarshal(body, &direct); err != nil {
		return nil, fmt.Errorf("decode body: %w (body=%s)", err, opsutil.Truncate(string(body), 200))
	}
	return &direct, nil
}
