...
func TestMarkChunkDone(t *testing.T) {
	t.Parallel()
	now := time.Date(2026, 5, 24, 12, 0, 0, 0, time.UTC)
	chunks := []opsutil.RangeChunk{{From: 2, To: 1_000_000}, {From: 1_000_001, To: 2_000_000}}
...
func TestResumeChunks_planDiffers(t *testing.T) {
	t.Parallel()
	now := time.Date(2026, 5, 24, 12, 0, 0, 0, time.UTC)
	priorChunks := []opsutil.RangeChunk{{From: 2, To: 1_000_000}, {From: 1_000_001, To: 2_000_000}}
