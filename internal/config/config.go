...
type AggregateConfig struct {
	...
	OutlierSigmaThreshold        float64                    `toml:"outlier_sigma_threshold" doc:"Reject a trade whose price is more than N robust scales from the per-window MEDIAN before VWAP. Implementation is a masking-resistant MEDIAN + 1.4826×MAD scale (internal/aggregate/outliers.go), NOT mean+stdev; 0 disables it and fewer than 3 valid prices is a no-op. Note the filter is volume-blind (one price per trade), so a large legitimate print far from a tight cluster can be dropped — see the aggregate audit." default:"4"`
