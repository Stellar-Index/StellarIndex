...
var Registry = map[string]Metadata{
	...
	"defindex":        {Class: ClassRouter, DefaultWeight: 0, IncludeInVWAP: false, Paid: false, BackfillAvailable: true, BackfillSafe: false /* decoder RE-DERIVED 2026-05-19 to the real on-chain schema ("BlendStrategy",deposit|withdraw){from,amount}, topic-dispatched across all emitters (the tag-1.0.0 "DeFindexVault" schema was fiction; deployed WASM 11329c24...988 is Blend strategy code). BackfillSafe stays false until live decoding is verified producing rows on r1 AND the WASM is re-audited vs 11329c24. Task #28; see docs/operations/wasm-audits/defindex.md */},
