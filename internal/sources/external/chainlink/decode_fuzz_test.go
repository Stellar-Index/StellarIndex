...
// fuzzNow puts the whole plausible updatedAt range inside the future-skew
// window, so these oracles test the word decoding alone.
var fuzzNow = time.Unix(maxPlausibleUpdatedAtUnix, 0)
...
func FuzzDecodeLatestRoundData(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, raw []byte) {
		rnd, err := decodeLatestRoundData("0x"+hex.EncodeToString(raw), "0xABC", fuzzNow)
		if err != nil {
			return
		}
		if len(raw) != 160 {
...
func TestDecode_RejectsUpdatedAtAboveUint64(t *testing.T) {
	...
	if rnd, err := decodeLatestRoundData("0x"+hex.EncodeToString(b), "0xabc", fuzzNow); err == nil {
	...
	if rnd, err := decodeAnswerUpdatedLog(entry, fuzzNow); err == nil {
...
func TestDecode_ZeroAnswerIsNonPositive(t *testing.T) {
	...
	if _, err := decodeLatestRoundData(raw, "0xabc", fuzzNow); !errors.Is(err, ErrNonPositivePrice) {
	...
	if _, err := decodeAnswerUpdatedLog(entry, fuzzNow); !errors.Is(err, ErrNonPositivePrice) {
...
func TestDecode_UpdatedAtBoundary(t *testing.T) {
	for _, tc := range []struct {
		...
		rnd, err := decodeLatestRoundData(raw, "0xabc", fuzzNow)
		if (err == nil) != tc.ok {
