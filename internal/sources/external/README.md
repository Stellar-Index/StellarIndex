...
| [`forex`](forex/) (`massive`) | Bespoke worker in the API binary (not an `external.Connector`); writes `fx_quotes` | Active fiat/USD reference feed behind the triangulation forex-snap | API key (`MASSIVE_API_KEY`) | Massive grouped-daily FX aggregates (the day's close); one row per ticker per UTC day; liveness via `stellarindex_external_fx_last_quote_unix`, not the `external_poller_*` family |
