-- 0175 down — drop `usd_volume_restamp_log`. REFUSES while it holds rows:
...
BEGIN
  IF EXISTS (SELECT 1 FROM usd_volume_restamp_log) THEN
    RAISE EXCEPTION '0175_usd_volume_restamp_log.down.sql: usd_volume_restamp_log still holds before-images — down-migrating would make every logged restamp irreversible. Export or delete them explicitly first.';
  END IF;
END $$;
