...
mkfixture() {
  local num="$1" name="$2"
  # 0001 stays in every scenario below, so hanging the fixture's one
  # create_hypertable() call off it keeps lint-migrations.sh's
  # hypertable-index pass (it fails vacuous with none under MIG_DIR)
  # satisfied without touching the back-number scenarios themselves.
  if [ "$num" = 0001 ]; then
    printf -- "-- fixture\nSELECT create_hypertable('dummy', 'id');\n" >"$TMP/migrations/${num}_${name}.up.sql"
  else
    printf -- '-- fixture\nSELECT 1;\n' >"$TMP/migrations/${num}_${name}.up.sql"
  fi
