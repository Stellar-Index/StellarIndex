#!/usr/bin/env bash
# prometheus-firewall-ipv6-test.sh — regression test for SL13.
#
...
# prometheus_internal_cidrs or prometheus_allowed_ssh_cidrs got no error
# and no rule: the entry just vanished from the rendered ruleset, so
# IPv6-only peers had no path in even though the inventory said they
# should. This renders the SAME Jinja content nftables.conf is built
# from and asserts an IPv6 CIDR lands in a v6 set AND an accept rule.
set -euo pipefail

cd "$(dirname "$0")/../.." || exit 1
TASKS="configs/ansible/roles/prometheus/tasks/06-firewall.yml"

if [ ! -f "$TASKS" ]; then
  echo "prometheus-firewall-ipv6-test: FAIL — missing $TASKS" >&2
  exit 1
fi
...
if [ -z "$PY" ]; then
  if [ "${CI:-}" = "true" ]; then
    echo "prometheus-firewall-ipv6-test: FAIL — no python3 with jinja2 (required in CI)" >&2
    exit 1
  fi
  echo "prometheus-firewall-ipv6-test: SKIP (no python3 with jinja2 locally; CI enforces)"
  exit 0
fi
...
if not m:
    print("prometheus-firewall-ipv6-test: FAIL — could not locate the nft `content:` block", file=sys.stderr)
    sys.exit(1)
lines = m.group(1).splitlines()
nft_src = "\n".join(l[6:] if l.startswith("      ") else l for l in lines)

rendered = jinja2.Template(nft_src).render(
    prometheus_internal_cidrs=["10.0.0.0/8", "2001:db8:1::/64"],
    prometheus_allowed_ssh_cidrs=["10.0.0.0/8", "2001:db8:2::/64"],
...
if failed:
    sys.exit(1)
print("prometheus-firewall-ipv6-test: PASS")
PYEOF
