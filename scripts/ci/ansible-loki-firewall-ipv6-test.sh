#!/usr/bin/env bash
# ansible-loki-firewall-ipv6-test.sh — pins SL12: server-05-firewall.yml
# built its nftables allow-sets with `if ':' not in cidr`, so any IPv6
# CIDR in loki_internal_cidrs / loki_allowed_ssh_cidrs was silently
...
# IPv6 internal CIDR for Loki ingest, would see it accepted by ansible
...
cd "$(dirname "$0")/../.." || exit 1
TASKS="${TASKS:-configs/ansible/roles/loki/tasks/server-05-firewall.yml}"
...
if [ ! -f "$TASKS" ]; then
  echo "ansible-loki-firewall-ipv6-test: FAIL — $TASKS not found" >&2
  exit 1
fi
...
check_set "internal_v6" "loki_internal_cidrs" ":"
check_set "ssh_allowed_v6" "loki_allowed_ssh_cidrs" ":"
...
if grep -qE 'tcp dport \{\{ loki_http_port \}\} ip6 saddr @internal_v6 accept' <<<"$body"; then
  ok "chain input: loki ingest accept rule matches ip6 saddr @internal_v6"
else
  bad "chain input: no loki ingest accept rule matching ip6 saddr @internal_v6 — IPv6 CIDRs are silently unenforced"
fi

echo "ansible-loki-firewall-ipv6-test: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
