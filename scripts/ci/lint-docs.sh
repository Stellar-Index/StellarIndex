...
if [ -d internal/api/v1 ] && [ -f openapi/ratesengine.v1.yaml ]; then
  grep -rE "^\s*(Get|Post|Put|Delete|Patch)\(" internal/api/v1/ 2>/dev/null | \
    sed -E 's|.*"(/v1[^"]*)".*|\1|' | sort -u | while read -r route; do
      ...
      if ! grep -qE "^\s+\"$route\"" openapi/ratesengine.v1.yaml; then
...
find docs/reference -type f -name '*.md' 2>/dev/null | while read -r f; do
