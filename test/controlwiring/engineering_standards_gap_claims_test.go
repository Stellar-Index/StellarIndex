...
func TestEngineeringStandardsDoesNotClaimUnbuiltEnforcement(t *testing.T) {
	...
	for _, c := range cases {
		...
		if !ok {
			t.Fatalf("heading %q not found — has it moved or been renamed?", c.heading)
		}
...
func TestEngineeringStandardsSection6DoesNotClaimUnbuiltEnforcement(t *testing.T) {
	...
	cases := []struct {
		label            string // section 6 table row's first column
		...
		},
		{
			...
			unfixedMechanism: "CI scan of flag registry",
		},
	}

	for _, c := range cases {
		rowRE := regexp.MustCompile(`(?m)^\|\s*` + regexp.QuoteMeta(c.label) + `\s*\|.*$`)
		row := rowRE.FindString(doc)
		if row == "" {
			t.Fatalf("section 6 table row for %q not found — has it moved or been renamed?", c.label)
		}

		if _, err := os.Stat(filepath.Join(root, c.artifact)); err == nil {
			t.Fatalf("%s now exists — the %q row is no longer aspirational; "+
				"update the table to name the mechanism plainly and delete this gap check", c.artifact, c.label)
		}

		if strings.Contains(row, c.unfixedMechanism) {
			t.Errorf("section 6 table still claims %q is enforced via %q, but %s does not exist in the repo:\n%s",
				c.label, c.unfixedMechanism, c.artifact, row)
		}

		if !strings.Contains(strings.ToLower(row), "gap") {
