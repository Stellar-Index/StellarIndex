...
// 0051's stored table comment, verbatim; 0180's down must restore exactly this.
...
// after migration 0180 up and down (GH #923): the indexer writes the row
...
func TestLedgerIngestLogComment(t *testing.T) {
	...
	if err := m.Migrate(179); err != nil {
		t.Fatalf("migrate down to 179: %v", err)
	}
	table, col = ledgerIngestLogComments(t, ctx, db)
	if table.String != ledgerIngestLogComment0051 {
		t.Errorf("0180 down did not restore 0051's table comment verbatim:\n got %q\nwant %q", table.String, ledgerIngestLogComment0051)
	}
	if col.Valid {
		t.Errorf("0180 down left a persisted_at comment 0051 never set: %q", col.String)
	}
}
