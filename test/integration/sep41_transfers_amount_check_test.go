...
// TestSEP41TransfersAmountCheck executes 0168 (T090): before it the table
...
// a compressed chunk, and the down restores the pre-0168 schema.
func TestSEP41TransfersAmountCheck(t *testing.T) {
	...
	if err := insertSEP41TransferAmount(ctx, db, 0, "transfer", "-1"); err != nil {
		t.Fatalf("pre-0168 negative transfer amount was refused (%v): the schema already had a CHECK and this test would be vacuous", err)
	}
	if _, err := db.ExecContext(ctx, `DELETE FROM sep41_transfers`); err != nil {
		t.Fatalf("clear pre-0168 row: %v", err)
	}
	...
	if compressed == 0 {
		t.Fatal("no compressed sep41_transfers chunk after 0168 — the ALTER was not exercised against compressed data")
	}
	...
	for i, c := range refused {
		...
		if !errors.As(err, &pgErr) || pgErr.Code != "23514" || pgErr.ConstraintName != "sep41_transfers_amount_check" {
			t.Errorf("%s amount %q after 0168: err = %v, want check_violation on sep41_transfers_amount_check", c.kind, c.amount, err)
		}
	}
	...
	for i, c := range accepted {
		if err := insertSEP41TransferAmount(ctx, db, 20+i, c.kind, c.amount); err != nil {
			t.Errorf("%s amount %q after 0168 refused: %v", c.kind, c.amount, err)
		}
	}

	applyMigrationsUpTo(t, dsn, 167)
	if err := insertSEP41TransferAmount(ctx, db, 30, "transfer", "-1"); err != nil {
		t.Fatalf("after 0168 down, negative transfer amount still refused: %v", err)
	}
}
...
// compresses it, the shape production is in when 0168 applies.
