...
- ⚠ **REFRESHED 2026-08-29 (recipe-fold before Wave D) against commit `51dfeb34`; prior 2026-08-14 (audit №4) at `d30408e9` — read
...
| AGT | **the audit-2026-07-23 remediation** (9 PRs / 192 commits / 743 files — largely agent-written, see §9); anything doc-says-X-code-does-Y | | newest surface |
...
17. **CH skip-index FP rate sets a pruning FLOOR** — `ledger_entry_changes` is 150.78B rows / 6.17 TiB with `INDEX idx_lec_key_xdr key_xdr TYPE bloom_filter(0.01) GRANULARITY 1`. At FP=0.01 over 18,405,620 granules the index cannot survive fewer than ~1% of them *however selective the key is*: measured 172,942 granules → 1.38B rows / 101.58 GiB / 21.5s to return **4 rows**, so `/contracts/{id}/code-history` 500s at its 8s deadline for every contract. **Check the arithmetic (`FP × granules` vs expected result size) before blaming any query on this table** — no rewrite can beat the floor. Same question applies to `idx_lec_account_id` and `idx_lec_asset`, both also 0.01. *Status: `idx_lec_key_xdr` retuned to `bloom_filter(0.0001)` in `deploy/clickhouse/tier1_schema.sql` on 2026-07-24 (~100× tighter floor, measured 1.90× index size); r1 still runs 0.01 until an operator executes `deploy/clickhouse/ledger_entry_changes_key_xdr_index_fp.sql`. The other two are deliberately untouched. Note ClickHouse has no `ALTER … MODIFY INDEX`, and the FP request bottoms out at 20 bits/row (~6.7e-5) — asking for less buys nothing.*
18. **Config drop-ins are ignored on a root-element mismatch, silently** — `/etc/clickhouse-server/*` is rooted `<clickhouse>` but the packaged `/etc/clickhouse-client/config.xml` is rooted `<config>`. A `<clickhouse>`-rooted client drop-in parses fine, is never merged, and reports nothing. This bit for 7 weeks: MinIO owns `:9000` on r1 (CH native was moved to **9300**), so bare `clickhouse-client` connects to MinIO's S3 API, sends a ClickHouse handshake and **hangs forever** — no error, no log, and five runbooks tell an operator to run exactly that. Always assert config *effect* (run the bare command) not config *contents*.
19. **The local gate is not CI's gate** — `make verify` calls itself "run before every push" but ran `gitleaks detect --no-git` (working tree) where CI runs `gitleaks detect` with `fetch-depth: 0` (**committed history**), and ran none of CI's three generated-artifact drift checks (`docs/reference/api`, `examples/postman`, `web/explorer/src/api/types.ts`). A fixture committed then deleted passes locally forever while CI stays permanently red. Fixed 2026-07-25, but the *class* recurs whenever a CI job is added without a matching verify.sh step — and it matters more now that changes land on main by direct push, where CI is no longer a pre-merge backstop.
...
- **RFC-10 — Individually-green PRs, jointly red.** Tests that pin another PR's *prose* (promtool `description` text, #303 vs #297/#265/#270) or generated artefacts resolved by picking a side (Postman, #305). Detect: pin structure not prose; regenerate on the merge result; reproduce on a clean `origin/main` worktree before reading job logs.
- **RFC-11 — Restart of a resumable job re-derives from the start; a metric that only exists after first use.** Testnet `scan-and-fill --start 2` auto-restarted after its own 5G cap OOM and re-applied 2.25M ledgers while looking healthy; `gap_detector_runs_total` CounterVec absent after a restart → false "detector dead" (#323). Detect: monitors must alert on a progress counter going *backwards*; every labelled counter pre-registered at 0; memory caps sized for the end of the range.
- **RFC-12 — Check-mode is blind to runtime-computed change; a bounded fix hides a larger deficit.** `--check` showed no galexie restart handler, the apply restarted mainnet galexie (#307); the reflector-fx replay over the last window verified clean while the full-range verdict showed Δ=97,826 rows back to 61602787. Detect: first real run of a changed IaC path is itself a test — watch it; never trust a bounded re-derive until the full-range verdict says so.

## 9. Hot spots (audit first, hardest)

- **THE audit-2026-07-23 REMEDIATION (`f84e2d0b..b57e117d`, 743 files) — audit this FIRST.** Newest, largest, least-audited, and written under time pressure largely by agents. Its own REVIEW-AFTER set names the protected classes to re-derive: **migration 0115** (notional floor on all 7 OHLC caggs + open/close, serve-layer band removed — changes every bar on every pair, and is INERT until re-materialized); **VWAP direction union** (now volume-weighted, was trade-count — changes served prices on two-sided pairs); **the re-derive guard** (fail-closed at the trade-write choke point); **supply dormancy horizon** (operator-visible: dormant >24h now fails closed); **the completeness verdict** (per-target data-derived floor + migration 0116's durable floor — expect sources to flip to `complete:false` where real loss exists); **auth/ratelimit** (fail-closed on sustained Redis failure, IPv6 /64 keying, SEP-10 replay key format changed base64→hex = a one-time replay window at deploy); **CORS default** `["*"]`→`[]`; **CI gates** (cosign verify-blob, signed migration bundle, base-ref gate-script restore).
- **The 2026-07-10 push** (two-axis verdict, verify-lake/hashchain/contiguity, reconcile-balances, ced-v2, ADR-0047→0048 classic-movements, CAP-33): was the 2026-07-16 run's newest surface. Now once-audited — lower priority than the remediation above, higher than settled code.
...
- **Explorer reads over `stellar.ledger_entry_changes` (150.78B rows / 6.17 TiB)** — the table is sorted `(ledger_seq, tx_hash, op_index, change_index)`, so EVERY lookup by `key_xdr` / `account_id` / `asset` rides a `bloom_filter(0.01)` skip index whose pruning floor is ~1% of 18.4M granules ≈ 1.4B rows (trap 17). `/contracts/{id}/code-history` was measured 500ing at its 8s deadline for every contract because of this. **Audit the sibling readers on the same indexes** (`idx_lec_account_id`, `idx_lec_asset`) — they have the same floor and their symptoms will be latency, not errors, until a deadline is added.
- **Probe served endpoints against R1 before believing a latency/status finding.** Cheap and repeatedly decisive: of the four endpoints in the C-F1/C-F2 cluster ("500 for all inputs", "15s no cap"), live testing showed `/accounts/{g}/operations` **200 in 5.2s**, `/accounts/{g}/transactions` **200 in 5.9s**, `/lending/pools/{p}/reserves` **200 in 0.8s**, and only `/contracts/{id}/code-history` actually 500ing. The API listens on **127.0.0.1:3000** (not 8080), and `clickhouse-client` needs **`--port 9300`** unless the trap-18 drop-in is in place.
- **Prior-audit re-verification set** (fast-shipped remediation is highest-risk; re-derive against current code):
  - *from audit-2026-06-30:* CS-010 (XLM mcap), CS-012/013 (SSE crash/FD), CS-017 (stale VWAP), CS-100 (org_verified), CS-124 (dashboard CSRF), CS-118/119 (root services/user), CS-009 (OG SSRF), CS-008 (SQL tenant scoping), CS-021 (ledger_entries_current ordering).
  - *from audit-2026-07-23 (verify these landed AND that nothing regressed):* A-CRIT-1/2 (re-derive NULL-usd_volume guard), A-CRIT-3 (backfill hang), REL-06 (rate-limit fail-open), MNY-05 (on-chain 1e7 decimals), MNY-06 (InvertScaled rounding + direction combine), B11-F1/MNY-01 (OHLC notional floor), N-F2 (completeness floor + migration 0116), COR-09 (projected-rebuild checkpoint), REL-08 (AsyncSink shutdown drain), AGT-12 (SSE + background-worker panic recovery), DAT-10 (RMT dedup — note TWO readers were MISSED by the first sweep, see RFC-12), CID-09/14 (cosign + migration compat), SEC-01 (auth infra exemption), DAT-09 (CAP-0038 legs; archive→live seam).
  - **Known-deferred, still open:** the R-072/R-078 phase-2 freeze calibration (coupled with COR-14's conf≡0 — must be fixed as ONE decision), the `stale-data-alert` producer gap's sibling metrics, `reliable-paging` (needs pager credentials), and the DAT-03 soroban_events `topics_xdr` historical recovery (no tool exists — see migration 0114's corrected note).

## 10. Audit waves & the surfaces each one covered

**Why this section exists.** Coverage is only meaningful as a cumulative claim. Without a per-wave surface map, the next audit either re-treads the well-worn paths or assumes a surface was covered because *some* audit covered *something* nearby. Every row records what was actually examined; the **gaps** at the end are what has never been audited by anyone.

| Wave | Scope examined | Output | Notes for the next run |
|---|---|---|---|
| **audit-2026-06-30** (CS-###) | First cold whole-system pass: money serve paths, SSE, CF edge functions, dashboard/auth, tenant isolation, root services | [`docs/audit-2026-06-30/`](https://github.com/Stellar-Index/StellarIndex/tree/0023bb9aefa96fb8231d9eabd160e6133eca39e9/docs/audit-2026-06-30) (coverage tracker + per-area files) | Source of the CS-### ids still cited throughout this file. Its Highs clustered in the newest surface (SSE, CF edge) — the origin of RFC-6. |
| **audit-2026-07-03-site** | Live site pass-B, product/UX surface | `docs/audit-2026-07-03-site/` (PLAN + REGISTER + PASS-B-FINDINGS) | Superseded by site-audit-2026-07-22 for the same surface. |
| **audit-2026-07-03-rfp-compliance** | RFP/contractual compliance claims | [`docs/audit-2026-07-03-rfp-compliance/README.md`](https://github.com/Stellar-Index/StellarIndex/blob/0023bb9aefa96fb8231d9eabd160e6133eca39e9/docs/audit-2026-07-03-rfp-compliance/README.md) | Narrow, one-off. Re-run only if the RFP claims change. |
| **audit-2026-07-16** | Cold multi-agent whole-repo + **the plan surface** (ADRs/ROADMAP/BACKLOG/perf-todo/architecture). Five recon agents: money-pricing · api-auth · ingest-completeness · config-deploy/entrypoints · web+plans | `docs/audit/audit-2026-07-16/` (findings, coverage-ledger, executive-summary, forward-path, recon/) | **§§1-6 of this recipe are that run's snapshot.** Its recon dumps are the evidence those sections distil. |
| **audit-2026-07-18-unaudited-surfaces** | Deliberately targeted the surfaces no prior wave had touched: deploy secrets, release/deploy pipeline, off-site backup, DR | `docs/audit/audit-2026-07-18-unaudited-surfaces/findings-ledger.md` | The model to copy when asking "what has nobody looked at?" — it found the deploy-secret blocker and the backup gap. |
| **site-audit-2026-07-22** (S1-S44) | Live site, **empirically probed not code-reviewed**: every page type, widget, link, load-timing, mobile, embeds | `docs/audit/site-audit-2026-07-22/findings.md` + `cold-console.mjs` | Carries a large *corrections-to-my-own-findings* trail — several findings were false and traced to TOOLING (headless `--window-size` is not device emulation; static-HTML greps false-positive on SPA routes). Verify UX/mobile/empty-state findings in a real rendered browser. |
| **audit-2026-07-23** (~480 confirmed) | Cold adversarial whole-repo in **6 chunks**: (1) money · (2) ingest/sources · (3) api/auth/security · (4) completeness/ops · (5) web/explorer · (6) plan/infra/config/CI | `docs/audit/audit-2026-07-23/findings.md` (+ remediation.md ledger) | The current corpus. Chunk 6 is marked PRIVATE — it names live infra. |
| **audit-2026-07-23 → wave-5 remainder** | The ~395 medium/low tail, re-partitioned into 10 batches by area: `api`(72) `auth`(12) `ci`(24) `cmd`(51) `docs`(25) `infra`(53) `ingest`(38) `money`(46) `ops`(45) `web`(29) | `docs/audit/audit-2026-07-23/wave5/*.json` | **Treat as hypotheses (RFC-11).** Measured hit rate on ~19 re-derived: ~50% already-fixed or refuted, 5 proposed remedies actively harmful. |
| **hardening-2026-08-28/29** (Waves A–C + Wave D early slice, 33 agents, 22 CONFIRMED) | Repo-side hardening (async sinks, bespoke cache clock, gap-detector census, gitleaks history, ansible galexie restart guard, drop-size guard, ops_batch identity), oracle capture-totality PR1–5 + composite §10 corroboration, runbook must-verify Waves D–J, deploy pipeline (ProxyJump, env-file reader, migrations single-archive sync), backups (pgBackRest repo2/S3 live, ZFS rolling snapshots, status-page backup panel), CI (4-way integration shards). | PRs #244–#324; `docs/operations/v1-launch-plan.md` addenda | Surfaces NOT re-audited cold since 2026-08-14: explorer/status web, API auth beyond self-mint, CH lake queries, aggregator pricing beyond composite. Next cold audit = Wave D on the post-#288 surface. |

### Surfaces NOT covered by any wave (the real gaps)

- **Live-infra runtime state** — every wave audited committed code. Only the 2026-07-25 remediation queried r1 directly, and doing so immediately surfaced three things static review had missed: TimescaleDB refresh jobs failing 37-69% on background-worker starvation; NVMe wear + apt metrics written every 5 min into a directory nothing scrapes; and three "fixed" symptoms still live because a deploy had not happened (RFC-9). **A future wave should audit the RUNNING SYSTEM as a first-class surface**: `pg_settings` vs the values the code assumes, `timescaledb_information.job_stats` failure counts, textfile-collector wiring, `schema_migrations` vs `migrations/`, the deployed tag vs `main`.
...
- **Backup / DR as *executed* rather than *designed*** — audit-2026-07-18 flagged the design gap; nobody has verified a restore. `restore-drill.timer` ships disabled, exactly one manual drill has ever run (2026-07-03), and the ClickHouse lake (8.6 TiB) had **no backup mechanism of any kind** until `ch-lake-backup` (ADR-0043 §2.4, 2026-09-23) — which backs nothing up on a host until its off-site target is configured, and has no restore drill yet. Audit the running host, not the job file: `stellarindex_ch_lake_backup_configured` and `_last_success_unix`.
...
- **Where audit outputs land:** `docs/audit/audit-<YYYY-MM-DD>/` — most recent is `audit-2026-07-23/` (findings + remediation ledger). NOTE these are **gitignored**: the repo is PUBLIC and findings/ledger content must never be committed. Commit CODE ONLY.
