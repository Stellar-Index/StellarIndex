...
- [ ] **Captive-core Tier-1 quorum was stale** — SDF's canonical `pubnet-validator-full` set moved to **18** (added **OBSRVR** ×3; dropped **SatoshiPay** ×3 + **Creit** ×3 validators, Creit kept as a validator-less HIGH home-domain); R1's live captive core still runs the old **21**. Codified in **PR #7** (`ops/captive-core-t1-validators-2026-07-17`) — verified renders 18/6-orgs, all strkeys pass CRC16, pre-change drift-clean. **Decision (2026-07-17): apply post-Phase-0** in the clean-release window (~07-20/21): a scoped galexie/captive-core config re-render + restart (`galexie.service` ~1–3 min live-tip pause; also `galexie-archive-fill.service`) — **Phase-0-safe** (CH re-derive reads the MinIO archive, not live core) and **no ops-binary swap**. Live tip carries the stale 6 until then (acceptable — core healthy 7d+). *Mid-Aug: Figure / Range / MoneyGram T1 additions — a second pass.*
