...
| `docs/{protocols,reference,methodology,blog,contributing,getting-started,engineering-standards}` | ~43 | Content + generated docs. `docs/reference` is generated (drift-gated in CI). Not deep-read; plans-docs covered adr/architecture/notes/perf-todo/AGENTS/README. |
...
| `docs/contributing/procedures` (9 project procedures), root files (LICENSE, CODEOWNERS, CODE_OF_CONDUCT, SECURITY.md, .gitignore) | ~14 | Meta/boilerplate. `docs/contributing/procedures` (the repo's own review/deploy procedures) not audited — low product risk. |
