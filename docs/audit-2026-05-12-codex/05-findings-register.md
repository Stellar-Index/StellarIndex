...
| F-1261 | high | Migration `0030_asset_supply_history_unique_constraint` cannot apply while `asset_supply_history` compression is enabled, so fresh integration bootstrap and the next R1 schema advance both fail | `migrations/0030_asset_supply_history_unique_constraint.up.sql`; `migrations/0005_create_asset_supply_history.up.sql`; migration runner; R1 schema state | XFI-0053; EV-0120 | fixed | db/release/ops | Wave 46 (2026-05-13) rewrites both up + down migrations to mirror the established compressed-hypertable pattern from `0004_relax_trades_ledger_for_offchain.up.sql`: `SELECT decompress_chunk(c, true) FROM show_chunks('asset_supply_history') c;` followed by `ALTER TABLE … SET (timescaledb.compress = false);` BEFORE the constraint DDL, and the original 0005 compression settings restored AFTER the swap. Fresh integration bootstrap now succeeds; the next R1 schema advance from version 28 → 30 will absorb cleanly on the next deploy. Migrations README entry updated to describe the new path accurately. |
