...
| F-1221 | medium | Active release docs still claim GHCR and arm64 artifacts that the current release workflow does not publish | Release workflow; release/deploy docs; Docker image expectations; repo orientation | XFI-0014; EV-0032; EV-0261 | fixed | docs/release | Closure was too narrow: `release-process.md` step 4 is fixed, but `CLAUDE.md` still says release tags cross-compile `linux/{amd64,arm64}` and push GHCR images, `release-process.md`'s pipeline summary still says `linux/amd64 + linux/arm64`, and `r1-deployment-state.md` still references a prior release with all 12 binaries. |
