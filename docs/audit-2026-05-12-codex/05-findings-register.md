...
| F-1228 | high | Source now clears SSE write deadlines, but live R1 tip streams still terminate around the old 30-second cutoff | API HTTP server; SSE stream endpoints; R1 live API | XFI-0020; EV-0038; R1-0016; EV-0119; R1-0020; EV-0141; EV-0166; R1-0023; EV-0173; R1-0028 | fixed | api/streaming/ops | Closed wave 130 + manual deploy to r1: rc.50 binary built locally and deployed (Actions billing cap blocked release.yml). Verified live: `curl --max-time 45 /v1/price/stream` held open the full 45s with curl exit 28 (client timeout), not exit 92 (server reset at 30.443s). SSE write-deadline cleared per `http.NewResponseController(w).SetWriteDeadline(time.Time{})`. |
...
| F-1306 | high | API price-stale alert is dead because `ratesengine_price_staleness_seconds` has no producer while R1 serves stale prices | API price handler; `internal/obs` metrics; Prometheus API alert; price-stale runbook; R1 metrics/status | XFI-0098; R1-0033; R1-0035; EV-0282; EV-0287 | fixed | api/observability/market-data | Closed wave 130 + manual deploy to r1: rc.50 binary built locally (Actions billing cap blocked release.yml), scp+atomic-swap on r1, restart. Verified live: `ratesengine_price_staleness_seconds{asset="crypto:BTC"} 0` etc. visible at aggregator :9465/metrics. The 120s alert query now has a producer with 3-asset cardinality (BTC/ETH/XLM bounded allow-list). |
