...
| F-1306 | high | API price-stale alert is dead because `ratesengine_price_staleness_seconds` has no producer while R1 serves stale prices | API price handler; `internal/obs` metrics; Prometheus API alert; price-stale runbook; R1 metrics/status | XFI-0098; R1-0033; R1-0035; EV-0282; EV-0287 | open | api/observability/market-data | Source now contains an aggregator-side bounded producer, but direct R1 verification still shows no `ratesengine_price_staleness_seconds` series in Prometheus or aggregator `/metrics`; live alert coverage remains absent until the fix is deployed/proven. |
