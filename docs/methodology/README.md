---
title: Pricing methodology
last_verified: 2026-09-23
status: current
---

# Pricing methodology

Public, customer-facing documentation of how Stellar Index turns raw
on-chain and off-chain trades into the prices served by the API. These
pages are written to be **honest about what a number means** — including
where a value is undefined, structurally delayed, or deliberately
excluded.

| Page | Covers |
|---|---|
| [vwap-aggregation.md](vwap-aggregation.md) | VWAP computation, the source-class policy (only exchange trades vote), stablecoin fiat-proxy late-binding, σ-outlier filtering, triangulation, closed-bucket serving, and the two freshness contracts (`/v1/price/tip` ≤30s vs `/v1/price` 30–150s) |
| [twap-ohlc.md](twap-ohlc.md) | TWAP + OHLC computation and the "no trades in window" contract (404, not a fabricated/LKG value) |
| [local-currency-pricing.md](local-currency-pricing.md) | How a price in a non-USD currency is produced — observed where a market exists (USD/EUR/GBP), otherwise `price(asset, USD) × rate_usd[ccy]` from the FX feed, labelled `flags.triangulated: true`; the daily-FX freshness caveat and why it is not a settlement rate (ADR-0051) |
| [dex-tvl.md](dex-tvl.md) | How per-protocol DEX TVL and the headline total are computed — which reserve source each AMM uses, the exclusion rules that keep a withheld, unpriceable or undecodable leg from inflating the number, the documented scope exclusions (classic CAP-38 pools, the SDEX order book, lending supplied-value, vault AUM), the admission check that refuses a carried-forward or unbalanced figure rather than serving a wrong total, and the per-pool drill-down (`/v1/protocols/{name}/tvl`) whose legs name the served-price identity and basis each was valued under or the reason it was excluded — with money rounded once, at the leg, so every level is the exact sum of the level below |
| [rwa-definition.md](rwa-definition.md) | What counts as a tokenized real-world asset — the four conjunctive requirements (identity as `(code, issuer)`; an issuer-bound SEP-1 `[[CURRENCIES]]` entry served from the on-chain `home_domain`; independent recognition of the issuer account in the curated directory with no scam-class tag; a real-world instrument class or an ADR-0028 oracle feed), the measured evidence for why a self-declaration alone admits a directory of exchange impersonators, what a failing asset gets (absence, counted and published), and why a withheld or unavailable valuation renders as unavailable rather than as zero |
| [listing-priced-valuation.md](listing-priced-valuation.md) | How a verified-catalogue asset whose market cap this index declines to publish can still carry a valuation from an independent listing platform — the two exact address routes (classic `CODE-GISSUER`, or the SAC deterministically derived from that (code, issuer)) and why a code match is worth nothing on a network carrying impersonators of PYUSD, USDT, USDC and XLM; why the figure is served under its own name with `provenance: listing_platform_price` and never inside `market_cap_usd`; why an observed market price always wins; why the supply must come from the lake's mint−burn total rather than a trustline sum (USDT0: 6,469 tokens against 2,581,052); and the fail-closed bounds that publish nothing rather than a stale price |
| [contract-storage-supply.md](contract-storage-supply.md) | How a token that emits no SEP-41 events reports the supply it actually holds — summed from the per-holder balance entries in its own Soroban contract storage under `supply_basis: contract_storage_balances`; why that is a DIFFERENT basis (distribution, a level) from the event-derived one (issuance, an accumulation) and why the two are never summed; the instrument check where three event-emitting tokens reproduced their event totals exactly and a Stellar Asset Contract disagreed by 6.8x (and is therefore refused); why the figure is a lower bound blind to TIME rather than to holding domains; and why the scale is read from the chain rather than borrowed |
| [xlm-circulating-supply.md](xlm-circulating-supply.md) | How XLM `circulating_supply` / `market_cap_usd` are computed (total − SDF non-circulating holdings), and the live reconciliation showing 0.03% agreement with CoinGecko + the Stellar Network Dashboard |

Related, non-public references:

- `docs/architecture/aggregation-plan.md` — the internal binding spec
  (policy chain, config surface, metrics, alerts).
- `docs/architecture/freshness-definition.md` — the two-contract
  freshness design.
- [Per-protocol verification pages](../protocols/README.md) — which
  sources feed the exchange class, and the contract-identity gating
  (ADR-0035) that makes each trade trustworthy.
- [d7-thin-pool-third-alias-vwap-review-2026-09-04.md](d7-thin-pool-third-alias-vwap-review-2026-09-04.md)
  — internal audit of every alias-walking price read path; residuals R1-R8.
