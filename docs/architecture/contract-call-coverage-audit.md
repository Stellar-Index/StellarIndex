...
> **Status (2026-09-02): every gap listed below has since closed — do not action this doc.** The "Phase-1 decoder not yet wired" rows for `cctp` and `rozo` are stale: both are **projected** sources, i.e. `cctp.Event` and `rozo.Event` are arms of `IsProjectedEvent` (`internal/pipeline/sink.go:499`) and `internal/projector` is their sole writer (ADR-0031/0032). The "real decoder gap: soroswap-router (8,729× undercount)" is closed too — `soroswap_router` is a wired `ContractCallDecoder` (AGENTS.md invariant 7; log-only, deliberately outside `IsProjectedEvent`). Kept as the evidence trail for how the gaps were found.
