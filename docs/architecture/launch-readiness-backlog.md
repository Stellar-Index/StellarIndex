...
| L2.12a | **Wire LCM-based supply observers into the indexer dispatcher.** The six observers (`internal/sources/{accounts,trustlines,claimable_balances,liquidity_pools,sac_balances,sep41_supply}`) all implement `dispatcher.LedgerEntryChangeDecoder` (or the contract-event sibling), but **no production code calls `disp.AddEntryDecoder` for any of them**. `pipeline.BuildDispatcher` only registers the trade/oracle decoders (soroswap, aquarius, …); the supply observers are orphan packages today. Effect: `account_observations`, `classic_supply_*_observations`, and `sep41_supply_events` hypertables stay empty in production, so the supply pipeline reads no input → `circulating/total/max_supply` and the F2 fields on `/v1/assets/{id}` are null in production despite the math being sound. The persistence side (`pipeline.Sink`) is already wired — it dispatches on observer event types and calls the right `store.Insert*` methods. The fix is operator-config-aware registration of each observer in `pipeline.BuildDispatcher` (or a sister helper called by `cmd/ratesengine-indexer`). One follow-up PR per observer keeps the change reviewable. | Wk 7 | ~2 days | L2.12 | L3.5 | `cmd/ratesengine-indexer/main.go` + `internal/pipeline/dispatcher.go` | 🔴 |
