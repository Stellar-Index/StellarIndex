...
| L4.11 | Public status page at `status.ratesengine.net` — shipped as a self-hosted Next.js 15 static export under [`web/status/`](../../web/status/README.md) deploying to Cloudflare Pages on every push to `main`. Incidents are Markdown files under `internal/incidents/data/<YYYY-MM-DD>-<slug>.md`, embedded into the API binary via `go:embed` so `ratesengine-ops emit-incident` fires customer-webhook fan-out (`incident.sev1` / `incident.resolved`, F-1249) from the same source. End-to-end operator procedure in [`runbooks/sev-status-page-update.md`](../operations/runbooks/sev-status-page-update.md). Earlier plans named Upptime + GitHub Pages then Instatus; both were superseded — see [`status-page-hosting-comparison.md`](status-page-hosting-comparison.md) for the decision history. (F-1211, 2026-05-13: row text rewritten to match the shipped path; Upptime/cstate references retired.) | Wk 9 | shipped | L4.4 | launch | `web/status/` Cloudflare Pages + `internal/incidents/data/` corpus | ✅ |
