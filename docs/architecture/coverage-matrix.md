...
`https://api.stellaratlas.xyz` v0.5.0-rc.39. See
...
|---|---|---|
| ✅ verified live | 28 | Curl returns the expected wire shape from `api.stellaratlas.xyz` |
...
- **S9.4** "three aggregators … Chainlink path remains
  ...
  `cmd/stellaratlas-api/main.go::buildDivergenceReferences` is
...
X3.1–X3.4/.6/.7; `cmd/stellaratlas-ops verify-archive -tier
...
| # | Requirement | Proposal | Week | Owner | ADR | Verified by | Status | Conf | Prod |
| - | ----------- | -------- | ---- | ----- | --- | ----------- | ------ | ---- | ---- |
| S5.1 | Live event ingest (Galexie/MinIO + ledgerstream + dispatcher) | §Real-time — Hot path | 3 | `cmd/stellaratlas-indexer` + `internal/ledgerstream` + `internal/dispatcher` + `internal/sources/*` | — | [data-sources/archival-nodes.md](../discovery/data-sources/archival-nodes.md), [ingest-pipeline.md](ingest-pipeline.md) | ✅ verified | 5 | ✅ 2026-05-10 — `latest_ledger=62,510,233` per `/v1/network/stats`; `markets_count_24h=23,646` |
| S5.2 | ≤ 30s staleness (Freighter SLA) | §Latency Targets | 6 | `cmd/stellaratlas-sla-probe` + `configs/healthchecks/stellaratlas-sla-probe.{service,timer}` | — | [data-sources/archival-nodes.md](../discovery/data-sources/archival-nodes.md) + HA plan | ✅ verified — `stellaratlas-sla-probe` measures `observed_at` freshness against the 30s target every 15 min; alerts in `deploy/monitoring/rules/sla-probe.yml` page on sustained breach. | 4 | ✅ 2026-05-10 — `/v1/price/tip?asset=native&quote=fiat:USD.observed_at` lags `as_of` by < 5min (closed-bucket); freshness alert wired |
...
| # | Requirement | Proposal | Week | Owner | ADR | Verified by | Status | Conf | Prod |
| - | ----------- | -------- | ---- | ----- | --- | ----------- | ------ | ---- | ---- |
| S6.1 | Since-inception backfill (ledger 2 → today) | §Historical Data | 2 (scaffold), 5 (run) | `cmd/stellaratlas-ops backfill` | — | [data-sources/galexie.md](../discovery/data-sources/galexie.md) + [data-sources/stellar-data-lakes.md](../discovery/data-sources/stellar-data-lakes.md) | ✅ verified | 4 | ⚠ 2026-05-10 — endpoint shipped (`/v1/history/since-inception?asset=native&quote=USDC-G…` → 200) but the **earliest data point on r1 is 2026-05-03** (~7 days), not ledger 2. Backfill not yet executed for older periods. |
...
| F3.3 | Responsiveness | ≥ 99.9 % | §Availability | 8–9 | HA plan + `cmd/stellaratlas-sla-probe` | (HA plan) | ⚠ caveat — synthetic 2xx-success-rate measured per probe run; 99.9% target needs production traffic to verify operationally. The HA topology (ADR-0008) is what backs the number. | 3 | ⚠ 2026-05-10 — needs ≥ 30 days production data + multi-region (currently single-region R1 only) |
| F3.4 | Data freshness (price) | ≤ 30 s staleness | §Data Freshness | 3 (ingest), 8 (deploy) | `internal/consumer` StreamLive + `cmd/stellaratlas-sla-probe` | [data-sources/archival-nodes.md](../discovery/data-sources/archival-nodes.md) | ✅ verified — probe measures `observed_at` freshness against the 30s target; `_freshness_breach` alert pages on sustained > 30 s. | 4 | ✅ 2026-05-10 — `/v1/price/tip.observed_at` is the closed-bucket end (last 5 min); the rolling-window tip is fresh per request |
...
| # | Requirement | Proposal | Week | Owner | Verified by | Status | Conf | Prod |
| - | ----------- | -------- | ---- | ----- | ----------- | ------ | ---- | ---- |
| F5.1 | REST or GraphQL | §API Layer | 7 | `internal/api/v1` (REST) | (API design) | REST shipped; OpenAPI spec at `openapi/stellar-atlas.v1.yaml`. GraphQL not in scope. | ✅ verified | 5 | ✅ 2026-05-10 — `GET /v1/healthz` → 200; OpenAPI live at `https://docs.stellaratlas.xyz` |
...
| - | ----------- | --- | ---- | ----- | ----------- | ------ | ---- | ---- |
| X1.1 | Primary archive (galexie-archive) — every closed partition has 64,000 files | [ADR-0017](../adr/0017-archive-completeness-invariants.md) | 8 | `cmd/stellaratlas-ops` + `galexie-archive-fill` | bootstrap completed 2026-04-28; all 17 previously-partial partitions filled | ✅ verified | 5 | 📦 ops-only — verified by archive-completeness daemon |
| X1.2 | Primary archive — chain-link integrity for every (N, N+1) | [ADR-0017](../adr/0017-archive-completeness-invariants.md) | 8 | `cmd/stellaratlas-ops verify-archive -tier chain` | `verify_archive_chunks.go` shipped; verifier running on r1 | ✅ verified | 4 | 📦 ops-only |
...
| X1.7 | `verify-archive` hardened: `checkpointsMissed > 0` is hard failure | [ADR-0017](../adr/0017-archive-completeness-invariants.md) | 8 | `cmd/stellaratlas-ops/main.go` | `-fail-on-missed` flag wired (`fs.Bool("fail-on-missed", ...)`); `checkpointsMissed > 0` returns non-zero exit when set, default-on per ADR-0017 X1.7 | ✅ verified | 4 | 📦 ops-only |
...
- **Closure**: backfill is operator-driven via `stellaratlas-ops
  backfill` (`cmd/stellaratlas-ops/backfill.go`); query performance
...
- **X1.5 archive-completeness daemon** — `cmd/stellaratlas-ops`
...
|---|---|---|---|---|
| Operations | Public status page at `status.stellaratlas.xyz` | infra | 9 | half-day |
...
| Connectors / Audit | Task #53 Blend Pool Factory walk on r1 (Phase 2 of audit) | `cmd/stellaratlas-ops wasm-history` | — | ~5 h operator |
...
- **2026-05-10** — **Production-verification column added.**
  ...
  status against `https://api.stellaratlas.xyz` v0.5.0-rc.39.
