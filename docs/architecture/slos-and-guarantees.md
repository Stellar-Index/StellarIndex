...
`docs/operations/sla-proof-<YYYY-MM-DD>.md`, generated by
[`scripts/ci/render-sla-proof.sh`](../../scripts/ci/render-sla-proof.sh)
and committed by `k6-weekly.yml`; each report names its own target, run
window, method, commit and instrument. **No such report exists yet** —
the load target secrets are unset, so the weekly run has nothing to
measure and correctly goes red. See
