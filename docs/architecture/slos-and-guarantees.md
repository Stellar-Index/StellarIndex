...
**Where the recurring proof lands.** Both figures above are one-off
captures recorded in prose. The repeatable trail is
`docs/operations/sla-proof-<YYYY-MM-DD>.md`; each report names its own
target, window, method, commit and instrument, and says which of two
generators wrote it.

The weekly one is generated by
[`scripts/ops/sla-proof-from-probe.sh`](../../scripts/ops/sla-proof-from-probe.sh)
and committed by `sla-proof-weekly.yml`, from the SLA probe's own series.
It measures **served latency and availability**, at concurrency 1, from
inside the API host — so it excludes DNS, TLS, the reverse proxy and any
CDN, and a client's p95 is strictly larger than the figures in it. It
bounds percentiles rather than averaging them and rolls nothing up across
endpoints; the report states all of this in its own text.

The load-at-volume one is generated by
[`scripts/ci/render-sla-proof.sh`](../../scripts/ci/render-sla-proof.sh)
from a k6 run, and **has no target to run against** — `K6_TARGET_STAGING`
is unset, and pointing the 300 rps soak at the single production host is
refused in code. That half of the claim is therefore unevidenced, and the
weekly report says so rather than implying its own numbers cover it. See
[`sla-proof-procedure.md`](../operations/sla-proof-procedure.md)
