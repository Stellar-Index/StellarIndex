...
- HAProxy backends are the 3 `stellarindex-api` pods.
...
{% for h in groups['stellarindex_api'] %}
    server {{ h }} {{ hostvars[h].ansible_host }}:{{ stellarindex_api_port }} check
{% endfor %}
