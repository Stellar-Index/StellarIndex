...
## 4. Capacity assessment for Stellar Index on this cluster

Can we slot the Stellar Index workloads into this cluster without
...
Phase A Stellar Index, R1-only, spread across pods on the 5-worker
...
| `stellarindex-indexer` | 2 | 2 GB | — | yes |
| `stellarindex-aggregator` | 2 | 4 GB | — | yes |
| `stellarindex-api` (3 replicas) | 3 × 1 | 3 × 2 GB | — | yes |
...
Ranked by "get Stellar Index running in Weeks 2–3":
...
- **Deploy our stellarindex-indexer + aggregator + api as new K8s
  manifests in a new `stellarindex` namespace** using the existing
...
- **Deploy brand-new** `stellarindex-stellar-core`, `stellarindex-
  galexie`, etc. in a fresh `stellarindex-infra` namespace with our
...
- **Stateful plane on Hetzner** (once approved) — stellar-core,
  ...
  spec.
- **Stateless plane in this cluster** — `stellarindex-api`,
  `stellarindex-aggregator`, observability, ingress. Use istio for
...
`stellarindex-indexer` can connect to
...
- Read-only network access from a new `stellarindex-indexer` pod in
  a new `stellarindex` namespace, pointed at the cluster-internal
