...
| A price value | **price** | **rate** — FX-vendor terminology only, inside the FX pollers (`internal/sources/external/ecb/`, `exchangeratesapi/`, `forex/`). NOTE: `RateLimit*` / `ratelimit` is UNRELATED (request throttling) — never sweep it in a rename. |
