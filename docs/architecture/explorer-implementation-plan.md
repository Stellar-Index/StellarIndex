...
| API types | **`openapi-typescript`** | Generates `web/explorer/src/api/types.ts` from `openapi/stellar-index.v1.yaml`. Single source of truth. CI check fails if regen drifts. |
...
| **API origin** | `api.stellarindex.io` (existing, behind Cloudflare per `cdn-setup.md`) | No change |
| **Showcase domain** | `stellarindex.io` (the explorer / showcase). The customer dashboard is in-site at `stellarindex.io/account` (the standalone `app.stellarindex.io` SPA was retired 2026-06-17 — see [`docs/operations/cf-pages-setup.md`](../operations/cf-pages-setup.md)). | CNAME → Cloudflare Pages OR A → r1; Cloudflare proxied (orange cloud) either way. Single-vendor CDN story. |
...
| # | Ticket | AC | Est |
|---|---|---|---|
| 0.1 | Bootstrap `web/explorer/` with Next.js 15 + TS + Tailwind + shadcn | `pnpm install && pnpm dev` runs, `/` returns 200 with "Hello, stellarindex" | S |
...
| 13.7 | Status page integration (link to status.stellarindex.io) | | S |
