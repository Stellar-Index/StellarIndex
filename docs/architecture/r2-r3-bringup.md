...
`cmd/stellaratlas-ops/cross_region_{check,monitor}.go`) just point
...
- R2 / R3 inventory entries don't exist; `groups['stellaratlas_api']`
...
2. **DNS.**
   - `api-r2.stellaratlas.xyz` → R2 host's IP.
   - Cloudflare LB record for `api.stellaratlas.xyz` adds R2 as a
     pool member with health check on `/v1/readyz`.

3. **Ansible inventory.**
   - Add R2 to `configs/ansible/inventory/multi-region/hosts.yml`
     under `stellaratlas_api`, `stellaratlas_aggregator`,
     `stellaratlas_indexer`, `redis_cluster`, `postgres_cluster`.
   ...
   - Vault: regenerate `redis_password`, `postgres_pass_*`,
     `stellaratlas_*_env` secrets and seed into `r2.secrets.yml`.
...
|-------|---------|----------|
| All regions serve | `curl -sf https://api-{r1,r2,r3}.stellaratlas.xyz/v1/healthz` | All 200 |
...
| Trade ingest parity | `stellaratlas-ops cross-region-check -metric vwap -window 1h -samples 24 -regions r1=…,r2=…,r3=…` | 0 divergences |
