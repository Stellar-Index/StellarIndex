...
The implementation lives in `cmd/stellarindex-ops/supply.go` (the
...
The CLI subcommand (`stellarindex-ops supply snapshot`) supports
...
`stellarindex_aggregator_supply_refresh_total{outcome=…}` counters.
