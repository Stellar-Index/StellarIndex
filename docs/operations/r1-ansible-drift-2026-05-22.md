...
(`stellarindex-archival-r1`, 136.243.90.96) against the
...
| `stellarindex.toml.j2` | **DRIFTED** — template far behind live config |
| `sshd_config.j2` | **NEVER APPLIED** — r1 runs stock Ubuntu sshd |
| `stellarindex-indexer/aggregator.service.j2` | **DRIFTED** — wrong `EnvironmentFile` path |
| `stellarindex-api.service` | **R1-ONLY** — no role template at all |
...
| `/etc/default/stellarindex` | **R1-ONLY** — role never creates the env file the services actually load |
...
### 2. `stellarindex-api.service` — no role template
The role's `tasks/14-stellarindex-services.yml` templates indexer +
aggregator only. No `stellarindex-api.service.j2`. A rebuild
produces no public API server. (`deploy/systemd/stellarindex-api.service`
exists as an operator-copied artifact.)

### 3. EnvironmentFile mismatch
Role unit templates reference `/etc/default/stellarindex-ops`; the
running indexer/aggregator/api units on r1 load
`/etc/default/stellarindex` — which the role never creates. That
file carries `STELLARINDEX_POSTGRES_DSN`, `AWS_*`, `MASSIVE_API_KEY`,
...
### 5. `stellarindex.toml.j2` — far behind live config
Live r1 `/etc/stellarindex.toml` has, beyond the template:
...
- [x] §3 add `/etc/default/stellarindex` env-file template + align unit `EnvironmentFile`
- [x] §2 add `stellarindex-api.service.j2` + install task
- [x] §5 backport live `stellarindex.toml` sections
...
- [x] §9 deployed `galexie-archive.yml` to r1 — `stellarindex.galexie_archive_tip_lag` (3 rules) loaded, prometheus SIGHUP-reloaded 2026-05-22
...
1. **Vault keys.** `templates/stellarindex.env.j2` + `galexie-backfill.toml.j2`
   reference vault vars that the real (encrypted) `inventory/r1.secrets.yml`
   must define: `postgres_pass_stellarindex`,
   `vault_stellarindex_reader_secret_key`, `vault_massive_api_key`,
...
| Template | Result |
| --- | --- |
| `stellarindex.toml.j2` | functionally byte-identical to live `/etc/stellarindex.toml` (only delta = the Ansible-managed header + comment placement) |
| `stellarindex-indexer/aggregator/api.service.j2` | **byte-identical** to r1's live units |
| `stellarindex.env.j2` | renders clean; all 10 keys match r1 (secret *values* are vault-sourced — unverifiable here by design) |
