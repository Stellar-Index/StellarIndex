...
- All sources healthy. `stellarindex_source_events_total` rate
...
| T+ | Beat |
| --- | --- |
| 0:00 | `stellarindex_ingestion_decode_error{source="soroswap"}` fires (>1/s for 5m). PagerDuty pages oncall (P3 → ticket; SEV-2 if it later escalates). |
...
| 8:00 | `stellarindex_aggregator_class_drop_spike` fires — VWAP for the affected pairs has lost soroswap as a contributor. |
...
- Confirm the issue is decoder-side, not source-side:
  `stellarindex_source_events_total{source="soroswap"}` is still
...
- Re-run the indexer over the gap range:
  `stellarindex-ops backfill -from <protocol-25-activation-ledger> -to <now> -source soroswap`.
...
| 7 | Did anyone propose using `stellarindex-ops backfill` to recover the gap window after the fix? |
...
- **Backfill subcommand is documented but might not exist.**
  The `decode-errors.md` runbook references `stellarindex-ops
  backfill` but the actual subcommand wiring is partial. Action
  item template: "Audit `stellarindex-ops backfill` for
