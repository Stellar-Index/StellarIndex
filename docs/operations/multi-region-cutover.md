...
- [ ] **Cloudflare zone for `stellarindex.io` under operator
      control.**
...
```sh
ssh r2 "systemctl status stellarindex-{indexer,aggregator,api}"
...
```sh
ssh r3 "systemctl status stellarindex-{indexer,aggregator,api}"
...
0. Pre-reqs (already done in cdn-setup.md):
   ...
   - DNS for stellarindex.io under Cloudflare control.

1. Per-region origin records:
   - api-r1.stellarindex.io → R1's HAProxy frontend IP
   - api-r2.stellarindex.io → R2's ALB / HAProxy frontend
   - api-r3.stellarindex.io → R3's HAProxy frontend
...
2. Cloudflare Load Balancer (Smart Routing / GeoIP):
   - Pool: stellarindex-api-pool
     - Origin 1: api-r1.stellarindex.io  weight=1
     - Origin 2: api-r2.stellarindex.io  weight=1
     - Origin 3: api-r3.stellarindex.io  weight=1
...
3. Public-facing record:
   - api.stellarindex.io → Cloudflare Load Balancer
...
while true; do
  date -u
  curl -sH "Host: api.stellarindex.io" \
    --resolve api.stellarindex.io:443:r2-ip-here:443 \
    https://api.stellarindex.io/v1/healthz \
