...
| P0-7 | **Source-catalogue: `massive` missing from `/v1/sources`** | [code] | ✅ **DONE** — bridged the active FX feed `massive` (the `internal/sources/forex` worker, `fx_quotes` path) into `external.Registry` as an external FX source. Now visible in `/v1/sources` + correctly `IsOnChain=false` (fixed a latent bug where it fell through to on-chain). `coinmarketcap`/`cryptocompare`/`exchangeratesapi` (and, at the time, a second FX connector for the same upstream as `massive`) confirmed as intentionally-present disabled **paid** connectors (honest catalogue). Needs an API deploy to show live. 2026-09-04: that duplicate FX connector has since been removed from the registry (#466); the disabled paid connectors are now `coinmarketcap`/`cryptocompare`/`exchangeratesapi`. |
...
- ~~**FX-path debt**~~ ✅ **DONE 2026-07-04 (BACKLOG #42)** — `FXQuoteAtOrBefore` now reads **`fx_quotes`-first** (the active `massive` feed; exact `*big.Rat` from `rate_usd` NUMERIC text, USD-anchored inversion/cross, 7-day lookback) and falls back to the legacy `trades`-path read only when `fx_quotes` has no row in the lookback — keeps the connector-path FX sources' compat if re-enabled. `AggregatorFXSnapFallbackTotal` should go quiet for massive-covered fiat legs after the next aggregator deploy. 2026-09-04: the former duplicate FX connector for the same upstream as `massive` was removed from the registry (#466); the compat path now covers `exchangeratesapi` alone.
...
| P1-2 | **`/v1/tx` `tx_hash_index`** — ordered lookup + MV + 10.2B-row backfill (perf-todo §4). | 🟡 **Code SHIPPED 2026-07-05** (schema + MV + two-step reader with scan fallback + `stellarindex-ops ch-txindex-backfill`). Remaining: apply the schema on r1 + run the windowed 10.2B-row backfill (operator-serialized; exact invocation in perf-todo §4). **Low launch priority** — the tx pages are noindex (UX latency only). |
...
| P4-2 | ~~**Supply observers** — ClassicSupplyReader production primitive (ADR-0022), SEP-41 per-contract event decoder (ADR-0023, currently a stub), AccountEntry reserve backfill (ADR-0021, interim static). Wire ADR-0011 SEP-1 `max_supply` overlay (dead code today).~~ ✅ 2026-07-05: three of four were doc drift — `StorageClassicSupplyReader` + the `sep41_supply` decoder/reader/refreshers shipped long ago (ADR-0022/0023 now carry status addenda), and the "interim static" reserve reader has been the chained live-LCM-first fallback since L2.12a. Real gaps closed: SEP-1 `max_supply` overlay wired into `/v1/assets/{id}` (`supply_basis: "sep1_declared_max"`), and `stellarindex-ops supply seed-observations` (lake-derived dormant-reserve-account bootstrap). | ADR-0011/21/22/23 | Operator follow-up: run `supply seed-observations` on r1 (after a state-snapshot account-scope run if reserves are pre-62M-dormant). |
...
| P4-4 | ~~**Anomaly Phase 2/3** — cross-oracle confidence factor (blocks on `internal/divergence` maturity); write `anomaly-freeze-engaged.md` runbook.~~ ✅ 2026-07-05: divergence maturity lifted by the five on-chain references (BACKLOG #31); cross-oracle agreement count computed in the refresh cycle + served via `confidence_factors.cross_oracle_checked`/`cross_oracle_agreement`; runbook rewritten against the shipped implementation (draft → ratified). | ADR-0019 | Phases 1–3 shipped. |
