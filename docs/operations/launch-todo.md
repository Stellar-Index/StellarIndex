...
| P2-4 | **Pricing polish** (3 sub-items). | r1-deploy-state | 🟡 [code] | **Verified 2026-06-30 (P2 goal):** (a) `/v1/price/tip` fiat:USD = **✅ STALE/RESOLVED** — `?asset=X&quote=fiat:USD` returns 200 with prices (BTC 58602, ETH 1570, XLM 0.18; param is `asset`+`quote`, not `pair`). (b) **USDC/USD≈1.0 synthesis = 🔴 REAL, NEXT STEP** — `/v1/price?asset=USDC&quote=fiat:USD` returns `price:null`. The aggregator maps USDC→USD (`internal/aggregate/stablecoin.go`), so USDC/USD→USD/USD has no trades → null. **Fix (precise):** helper `aggregate.FiatProxy(asset) → (proxyAsset, isStablecoin)` already maps USDC/USDT/…→fiat:USD (`internal/aggregate/stablecoin.go:50`). In the `/v1/price` + `/v1/price/tip` `ErrPriceNotFound` path (`internal/api/v1/price.go`, `price_tip.go::computeTip`), when `FiatProxy(asset)` is a stablecoin whose proxy == the requested `quote`, synthesize: try the asset's direct CEX rate vs USD first (don't hide a depeg), else return `1.0` with a `single_source`/peg flag. CHECK the import boundary first (`scripts/ci/lint-imports.sh`) — if api→aggregate is forbidden, lift `FiatProxy`/`stablecoinFiatProxy` into a shared low-level pkg (e.g. `internal/canonical` or a new `internal/stablecoin`) and call from both. Add a handler test. (c) `min_usd_volume=0` stop-gap — still TO VERIFY (search r1-deployment-state + the aggregator `MinUSDVolume` config). |
