...
- **Galexie archive trim — 🔵 IN PROGRESS (started 2026-07-26 13:46).** Deleting the 780 parity-verified partitions below ledger 49,984,000 (= 781×64,000, exact partition boundary, no straddle). Genesis `FFFFFFFF--0-63999` EXCLUDED (AWS lacks ledger 2). Method: per-partition `mc rm --recursive` via `/usr/local/sbin/galexie-trim-run.sh` (resumable via `/var/lib/ch-backfill/galexie-trim-done.txt`, defensive per-line genesis/config/above-cutoff refusals), log `/var/log/galexie-trim.log`, monitored. ~15s/partition, ETA ~3.25h. **The trim tool itself can't be used for bulk here — it has no lower bound so it would trim genesis, and --verify-upstream is ~96 days at this scale; --no-verify-upstream needs env `set -a; source /etc/default/stellarindex-ops; set +a` (the ansible-blessed interactive creds — the config's archivewriter identity 403s).**
  - **Safety net:** ZFS snapshot `data/minio@pre-trim-2026-07-26` (full rollback; 0B at creation, grows to ~3.7T as blocks are freed). Do NOT destroy until the 48h canary soak is clean.
  - **⚠️ STATE CHANGES made for the trim that must be REVERTED/COMPLETED:** (1) `galexie-archive-fill.timer` is STOPPED — **re-enable after trim** (`systemctl start galexie-archive-fill.timer`). (2) hot floor raised to `ARCHIVE_HOT_FLOOR=49984000` in `/etc/default/galexie-archive-fill` — also set `stellarindex_archive_hot_floor: 49984000` in the ansible inventory so the next ansible apply keeps it (else a render reverts to 2 and re-downloads 3.7T). (3) Pool free space will NOT rise during the soak (snapshot holds the freed blocks); the ~3.7T reclaim lands when the snapshot is destroyed post-soak.
  - **Next after trim done:** re-enable fill timer → sanity cold read (backfill a trimmed range, confirm `ledgerstream_tier_read_total{outcome="cold"}` increments) → arm 48h canary soak → destroy snapshot if clean → THEN the consolidated deploy batch (headroom now available for migration 0115).
