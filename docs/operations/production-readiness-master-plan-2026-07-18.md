...
- **Post-dedup completeness re-audit — 🔵 RUNNING** (`/usr/local/bin/lake-completeness-audit-v4`, log `/var/log/lake-completeness-audit-v4.log`, monitored): per-bucket declared-vs-actual for operations/operation_results/contract_events + LEC probe (transactions skipped — v3 proved exact). **This is the GATE before trim.** NOTE: operation_results is compared vs `op_count` so its known early-failed-tx deficit will surface as small mismatches — interpret against 14,647, don't rubber-stamp. Serving-plane alert flaps (latency, data_source_stale, phoenix/soroswap insert-stale) were merge-window transients — should quiesce now the chain is done; re-check.
