...
- **⚠️ `seed-sep41-genesis` DONE — but supply is only PARTIALLY fixed (correction).** The genesis-baseline `supply refresh: compute failed` errors ARE gone (0 confirmed) — that specific cause is fixed. BUT a SEPARATE supply-refresh condition still fires and likely keeps `supply_refresh_error_dominant` up: `supply refresh: rejecting snapshot — component ledger frozen past the dormancy horizon (stalled observer, not a dormant asset)`. **Needs investigation** (a genuinely stalled balance-component observer = real, vs a dormancy-detection false-positive on a very quiet asset). Not urgent — prices are fine, no traffic. Do NOT claim supply clean until this is dispositioned.
