...
**As of 2026-07-26 ~11:00 CEST (post-audit-remediation, dedup chain in tail):**
- **Audit remediation — ✅ COMPLETE (code).** Every actionable finding from the 2026-07-23 audit's 480-finding reconciliation is fixed, adversarially verified (fixer↔verifier panels, red proofs), and on main. Landed 2026-07-25/26: tail batches A–J + the deferred four + the coordination wave — highlights: freeze-ladder durability (migration **0119**, Redis flush no longer releases escalated freezes), stripe event claim (**0121**, exactly-one processor), durable login lockout + reaper (**0122**), ledger-wide three-phase entry-change walk (dispatcher+lake, P23 refunds; **0120** comment-only + `entry-walk-renumbering.md` runbook — positions are version-scoped, re-derives need the runbook), substrateClaim fail-closed latch, C4-085 coverage anti-join, `verify-usd-volume` (report-only; run on r1, read tier-3/4 spread, THEN calibrate), quota/audit/clamp/fanout/lockout counters + alerts. Main tip `3cebda7b`, CI + api-audit green. Still deferred (prerequisites): C3-081 stripe-reconcile (needs Stripe SDK + `[billing]` config seam), C1-024 fiat methodology split, C1-022 depeg band (needs stablecoin/USD venue pairs); `[OP]` decisions: 15 accepted-risk candidates, IP rotation + SSH CIDR narrowing.
- **Galexie → AWS-public cold tier — ✅ LIVE** (ADR-0027): cold reads from `aws-public-blockchain` (us-east-2, anonymous), hot floor knob in place (currently 2). Parity verified 781/781 partitions + 1000/1000 md5s; genesis partition `FFFFFFFF--0-63999` is OURS-superset (AWS lacks ledger 2) — **permanently excluded from trim**. LEC repair done (1,170,797 ledgers re-ingested, damaged_after=0). Trim boundary decided: **49,984,000** (raise hot floor in the SAME change).
- **Lake dedup chain — ✅ COMPLETE (2026-07-26 12:49, "ALL TABLES DEDUPED").** Per-partition `OPTIMIZE FINAL` removed the ~50% duplicate rows across all 5 lake tables. Post-dedup counts (sane, dup signatures gone): ledgers 63,655,672; transactions 10.30B; operations 23.84B; operation_results 23.84B (≈operations; early-failed txs have no results — known ~14,647 deficit, proven exactly); contract_events 12.35B. Pool **521 G → 2.42 T free** (~15.9 T used). Ingest healthy throughout (lag <10s).
- **Post-dedup completeness re-audit — ✅ PASSED (2026-07-26 13:31).** operations: 0 issues (exact); contract_events: 0 issues (exact); LEC probe: no missing tx-bearing ledgers; operation_results: deficit-only, and the deficit was PROVEN exactly = operations belonging to rejected txs (result_code ≤ -2, which produce no results) on buckets 22 (990=990) and 29 (37,849=37,849). Lake is complete + deduplicated. Trim gate GREEN.
- **Galexie archive trim — ✅ DONE + VERIFIED (2026-07-26 ~17:00).** Deleted all 780 parity-verified partitions below ledger 49,984,000 (= 781×64,000, exact boundary, no straddle), 0 errors. Verified: trimmed partitions empty; **genesis `FFFFFFFF--0-63999` intact (63,998 files — ledger 2 safe)**; above-cutoff partitions intact; 996→216 partition dirs. Reclaim = **1.07T** (not 3.7T — early history is sparse; most BYTES are in the dense Soroban era above the cutoff, which was kept), held by the snapshot until destroy. Method used: per-partition `mc rm --recursive` via `/usr/local/sbin/galexie-trim-run.sh` (the trim TOOL can't do bulk here — no lower bound so it'd trim genesis, and --verify-upstream is ~96 days at this scale). `mc` `local` alias = MinIO root = full perms.
  - **✅ Cold-read CONFIRMED post-trim:** `rehydrate-galexie-archive -from 5000000 -to 5000002` read all 3 trimmed ledgers from cold (`missing_in_cold=0`) in <1s. (The rehydrate PUT-to-hot 403'd — the stale-archivewriter write-identity issue, so those 3 stayed trimmed = no cleanup; rehydrate-based rollback is degraded until the archivewriter cred fix in the deploy batch. **ZFS snapshot is the working rollback meanwhile.**)
  - **🔵 Safety net HELD:** ZFS snapshot `data/minio@pre-trim-2026-07-26` (used=1.07T). **DO NOT destroy until a 48h canary soak (started ~17:00 2026-07-26, ends ~17:00 2026-07-28) is clean** — i.e. cold keeps serving + no incident needing the trimmed data. Destroying it reclaims the 1.07T to the pool. This is the only remaining trim step and it is deliberately deferred.
  - **✅ State changes handled:** fill timer RE-ENABLED (`active`, respects the raised floor); hot floor = 49,984,000 in `/etc/default/galexie-archive-fill` + ansible inventory var + `R1_INVENTORY_B64` secret (all consistent — no re-download risk). Pool free stays ~2.4T during the soak (snapshot holds the freed 1.07T); reclaim lands at snapshot-destroy.
...
### ✅ DEPLOY COMPLETE + VERIFIED — v0.21.0 live on R1 (2026-07-26 ~19:05)
- **All 6 binaries deployed** (fresh ~19:00 mtimes; ops reports v0.21.0), services active, **CH lag 6s**, schema **122/clean**, `/v1/price` serving fresh VWAPs. Two standing alerts already cleared on the new code (`dex_nonstandard_decimals_detected`, `supply_cross_check_divergence`).
- **🔵 0115 CAGG re-mat RUNNING** (`/usr/local/sbin/cagg-remat.sh`, log `/var/log/cagg-remat.log`, monitored) — repopulating OHLC/TWAP history (1m=7d, 15m=30d, 1h+=full). Until it finishes, /v1/ohlc + /v1/chart have only recent buckets. Verify those serve history when the log shows `ALL CAGG REMAT DONE`.
- **Deploy took 4 attempts, every failure caught r1-untouched or in a known state** (approval gate → relaxed; backup-gate dash bug → bash fix; dirty-116 ownership → recovered to 122). No silent partial changes at any point — the guardrails worked.

### ⚠ SEED BATCH — partial (2026-07-26 ~22:xx)
- **⚠️ `seed-sep41-genesis` DONE — but supply is only PARTIALLY fixed (correction).** The genesis-baseline `supply refresh: compute failed` errors ARE gone (0 confirmed) — that specific cause is fixed. BUT a SEPARATE condition keeps `supply_refresh_error_dominant` up — now ROOT-CAUSED: `supply refresh: rejecting snapshot — component ledger frozen past the dormancy horizon`. **The served-tier balance projection `account_observations` is FROZEN at ledger 63,632,946** (~30k ledgers / ~1.75 days behind the lake tip, snapshot_ledger 63,663,238, gap 19,291 > dormancy_horizon 17,280) — verified NOT advancing (max(ledger) delta=0 over 15s). CH ingest is at tip (~3s), so only the PG balance projection lagged. It stalled ~2026-07-25, BEFORE tonight's deploy/ops (so not caused by them). **The supply guard is working correctly** (refusing to publish supply on a stale component — not a bug). **Root cause is one of:** (a) the pending **D4 PG served re-derive / `projector-replay`** hasn't run (account_observations is batch-populated and simply stale) — LIKELY, matches the Phase D plan; or (b) the real-time observation writer stalled under the dev binary and v0.21.0 hasn't resumed it (a bug — it's still stuck post-deploy). **Disposition COMPLETE:** all price/DEX projector sources are healthy AT TIP (phoenix/soroswap/reflector-*/redstone/aquarius/blend/defindex/sorocredit: committed_to ~63,663,29x, lag=0, 0 perm_fails/quarantined) — so it is NOT a general projector failure and NOT a bug. `account_observations` is the CLASSIC-balance projection, batch-populated by the pending **D4 `projector-replay`** (Phase D §0) which hasn't run since ~2026-07-25 → ~1.75 days stale. The supply guard correctly refuses the stale-component snapshot. **Fix = run the pending D4 served-tier balance re-derive** (heavy — deferred to a careful pass; part of the served-tier population batch alongside seed-sac-balances). Residual to confirm at fix time: whether the real-time pipeline (`internal/pipeline/sink.go:1670` InsertAccountObservation) is meant to keep it at tip (then also investigate why it isn't) vs batch-only. Money-adjacent (§1 "served correctness" go-live item) but NOT urgent (prices fine, no traffic). NOT the genesis error (fixed).
- **✅ Re-mat DONE** — all 9 OHLC/TWAP CAGGs; /v1/ohlc serves history; coverage recovering.
- **✅ sep41 completeness root cause FIXED IN CODE (commit `06ff3b5e`), pending deploy.** `CountRowsByLedger`/`MinLedger`/source-coverage hardcoded a 5-min `SET LOCAL statement_timeout` that silently failed the verify on grown hypertables → raised to a generous 2h (`opsVerifyStatementTimeoutMS`). **Takes effect on r1 at the next deploy** (r1 still runs v0.21.0 with the 5-min cap, so sep41_supply/sep41_transfers stay projection_ok=f until then). After deploy, re-run `compute-completeness -ch -source sep41_supply -skip-substrate` (and sep41_transfers) — each ~40 min, will now complete and clear `completeness_incomplete`. FOLLOW-UP (perf, non-blocking): 40 min for a 9.3M-row count is slow — worth investigating chunk exclusion / parallelism later. Original diagnosis below.
- **⛔ (now fixed above) sep41 completeness verify BLOCKED (tool statement_timeout too short).** Full runs for the two latched sources (`sep41_supply`, `sep41_transfers`, projection_ok=f) time out: `CountRowsByLedger sep41_supply_events [50459813,63661414]: pq: canceling statement due to statement timeout (57014)`. **Diagnosis (complete):** the query is CORRECT (EXPLAIN = Parallel Index-Only Scan on the ledger index) but HEAVY — ~9.3M rows across 130 hypertable chunks in the Soroban range, ~40 min. The timeout is NOT from the role (`{synchronous_commit=off}`), db-role (`{jit=off}`), global (superuser sees `statement_timeout=0`), or DSN (`?sslmode=disable`) — so **compute-completeness sets its own statement_timeout in code**, too short for large hypertables. **Fix options (do carefully, own pass):** (a) make the completeness query statement_timeout configurable / raise it for CountRowsByLedger; or (b) chunk CountRowsByLedger per hypertable-chunk so each fits. Correct table names: `sep41_supply_events` (hypertable, 130 chunks), `sep41_transfers` (hypertable, 31 chunks — NOT `_events`). ANALYZE'd sep41_supply_events (parent reltuples=0, data in chunks). **Not urgent:** only these 2 sources incomplete; all others complete; no traffic.
- **NOT yet run:** `seed-sac-balances -full-history`, `ch-participant-backfill`, MATERIALIZE idx_lecur_account_id, bloom index. (Deferred — avoid piling heavy DB ops late; pick up carefully.)

### ▶ REMAINING to full production
1. **CAGG re-mat** finishes (in progress).
2. **`[OP]` ansible config apply** (vault secrets): archivewriter cred fix (unblocks rehydrate rollback), triangulation chains, z=5.0, cold-tier render. Also unblocks ansible-drift.
3. **sep41 FULL completeness runs** (no `-from`, INV-5 latch) + seed-sep41-genesis + seed-sac-balances `-full-history` + ch-participant-backfill + MATERIALIZE idx_lecur_account_id + bloom index → clears `completeness_incomplete` + `supply_refresh_error_dominant`.
4. **verify-usd-volume** first run (`-days 30`) → calibrate C4-055/066 alert.
5. **48h galexie soak** (ends ~2026-07-28 17:00) → `zfs destroy data/minio@pre-trim-2026-07-26` (reclaim 1.07T) + disable galexie-soak-check.timer.
6. **Rotate** ratesengine-admin + MinIO creds (exposed in session logs). **Re-arm deploy approval gate** at production launch (`gh variable delete DEPLOY_APPROVAL_RELAXED` + r1 Required-reviewers).

### (history) ▶ DEPLOY — migrations DONE (schema 122), binaries rolling (2026-07-26 ~18:00)
...
### (history) ⛔ First deploy attempt BLOCKED on the human-approval gate (2026-07-26 ~17:45)
...
3. **sep41 FULL completeness runs** (INV-5 latch needs one full run per source, no `-from`), seed-sep41-genesis, seed-sac-balances `-full-history`, ch-participant-backfill, MATERIALIZE idx_lecur_account_id, bloom index (C-F1b).
4. **Postgres restart** (max_worker_processes), remove tip-lag 240s drop-in, **rotate ratesengine-admin + MinIO creds** (exposed in session logs).
5. **verify-usd-volume** first production run (`-days 30`) → read tier-3/4 spread → calibrate the C4-055/066 divergence alert.
6. Post-deploy: the six standing alerts (completeness_incomplete, supply_cross_check_divergence, etc.) should finally clear once sep41 full runs + new-binary re-derives land.
- **Next (in order):** chain completes ("ALL TABLES DEDUPED") → post-dedup completeness re-audit (`/usr/local/bin/lake-completeness-audit-v4`, expects clean) → **trim** per `lcm-cache-tiering.md` (ZFS snapshot → hot floor 49,984,000 + per-partition wholesale delete from parity-verified list, genesis excluded → 48h canary soak vs `/var/lib/stellarindex/cold-canary-manifest.txt` → destroy snapshot) → **consolidated deploy batch**: ansible apply (chains, archivewriter cred fix, cipher ack, cold-tier render, z=5.0) + deploy all binaries + migrations **0115** (headroom now exists post-dedup; 1.1 TB re-mat) **+ 0117–0122** + sep41 FULL completeness runs (INV-5 latch) + seed-sep41-genesis + seed-sac-balances -full-history + ch-participant-backfill + MATERIALIZE idx_lecur_account_id + bloom index (C-F1b) + PG restart (max_worker_processes) + remove tip-lag 240s drop-in + rotate ratesengine-admin & MinIO creds (exposed in session logs).
- **ansible-drift secrets:** `R1_INVENTORY_B64` ✅ created 2026-07-26 (from local r1.yml). Drift now fails one step later on `ANSIBLE_VAULT_PASSWORD` + `ANSIBLE_VAULT_FILE_B64` — **still `[OP]`, cannot be set from inside the repo** (needs the live r1 vault password + the base64 vault file). Set both in repo Settings → Secrets → Actions, then re-run `gh workflow run ansible-drift.yml`. Workflow stays red until then; this does not block anything else.
- **⚠️ In-flight / DO NOT:** don't restart phase0 scripts (halted). schema_migrations=114 on R1 is BY DESIGN (0116 applied directly, IF NOT EXISTS). Dedup driver = `/usr/local/bin/lake-dedup-driver` + `/tmp/dedup-chain.sh`.

---

**Prior state (2026-07-20, Phase D1 archive-walk running — kept for history):**
- **Phase A (Capacity) — ✅ DONE.** All 4 big CH tables recompressed LZ4→ZSTD: **~3.8 TiB reclaimed** (LEC 4.93→3.75, operations 3.13→2.08, op_results 2.28→1.45, contract_events 1.24→0.56); **pool 94% → 75%, ~4.2 TiB free.** Temp settings reverted (150 GiB ceiling). Lossless, live healthy throughout.
- **Phase B (Enablement) — ✅ DONE** (deploy secrets + CF token set).
- **Phase C (deploy) — ✅ DONE + VERIFIED (2026-07-19 ~17:49).** Released **v0.17.0** (signed, sigstore) from main HEAD `e38bce37`; deployed via `deploy.yml` (region=r1, all 6 binaries incl. `stellarindex-ops`+`stellarindex-migrate`). Verified: **schema_migrations=114 (clean)**; all 6 binaries fresh; services active (sla-probe is timer-driven — normal); CH-lake tip advancing; **the new binary writes full-fidelity + non-zero `intra_ledger_seq`** (last 200 ledgers: 985K rows ~4,925/ledger, 295K non-zero ordinals). C2-4c fix live. (`intra_ledger_seq` column added pre-swap; `ops-ch` doesn't exist here — re-derive uses `stellarindex-ops` v0.17.0 directly.)
- **Phase D — 🔵 IN PROGRESS (2026-07-19 ~19:51): D1 archive-walk RUNNING** (`/usr/local/sbin/phaseD-backfill.sh`, log `/var/log/phaseD-backfill.log`, state `/var/lib/ch-backfill/phaseD-done-windows.txt` — FRESH, not the old done-windows). **Canary validated:** a degraded 1000-ledger range went 0→2.74M real-op rows + `intra_ledger_seq` (max 8288), ~4,390/ledger, in 13s. **TUNED for throughput (2026-07-19):** two CONCURRENT range processes (`phaseD-range.sh`) — recent `[54M→63.05M]` at `-parallel 8` (~86 ledgers/s sustained, dense — the long pole) + early `[2→38M]` at `-parallel 4` (~327 ledgers/s, sparse — 21/38 windows done). **Bottleneck = ClickHouse ingest throughput on the dense era (~380–400K rows/s)** — CONFIRMED: splitting the dense range into 2 concurrent halves didn't scale (~86→~93), so it's CH-server-side, not per-process. That's the hard limit. **ETA ~30h total** (was ~2–4 days serial). Deeper CH ingest tuning has uncertain payoff; ~30h is acceptable for a one-time comprehensive backfill. Resumable (per-range state files phaseD-recent.txt/phaseD-early.txt), disk-guarded, deprioritized (live ingest protected + advancing). **Remaining after D1:** D2 in-CH ordinal reproject `[38→54M]`+`[63M→tip]`, D3 `ledger_entries_current` reproject, D4 PG re-derives, census DELETE, then Phase E prove. **Original design/order (superseded by the running plan):** the degraded ranges `[genesis→38M]`+`[54→63M]` hold **only census `state` rows** (op_index=-1, tx_hash='', consumed by `ledger_entries_current_mv`) — **0 real-op rows**. Archive-walk full-fidelity rows (op_index≥0) would COEXIST with census (different RMT keys), so the clean approach is **drop-census-per-partition (DROP PARTITION — safe, no real-op rows to lose) → archive-walk with v0.17.0 ops → MV projects full-fidelity**. Order: (D2) in-CH ordinal reproject `[38→54M]`+`[63M→tip]` (rows present — cheap, no walk) FIRST; then (D1) per-partition drop+archive-walk the degraded ranges (canary-test 1 partition first); then (D3) `ledger_entries_current` reproject; (D4) PG served re-derives; then Phase E prove. ~1–2 wk, resumable, disk-safe (4+ TiB free). Start after Phase A recompress finishes (contract_events, ~1–2h).
...
- **Immediate next (in order):** D1 archive-walk running (~18–20h, 2 concurrent range procs — watched) → **D2** in-CH ordinal reproject `[38→54M]`+`[63M→tip]` → **D3** `ledger_entries_current` reproject → **D4** PG served re-derives (`projector-replay`) → census DELETE → **Phase E** prove (`reconcile-balances`/`verify-lake`/supply cross-check) → **Phase F** harden (HA, config drift, DR, off-site backup).
- **Check D1 progress:** `ssh root@136.243.90.96 'tail /var/log/phaseD-backfill.log; grep -c "window .* DONE" /var/log/phaseD-backfill.log; df -h /var/lib/clickhouse'`.
- **⚠️ In-flight / DO NOT:** do **not** restart `/root/phase0.sh` or `/root/phase0-seam.sh` (halted runaways). D1 drivers = `/usr/local/sbin/phaseD-range.sh` (2 procs, state `phaseD-recent.txt`+`phaseD-early.txt`) — resumable, disk-guarded, deprioritized. Temp CH merge settings already reverted (150 GiB / old_parts 480). pgbackrest prune **deferred** (only backup copy until S3).
...
- **[RESOLVED 2026-08-24, v0.41.1]** the synthetic USD-cross reference (PR #149) gives EUR/GBP-quoted pairs a second lens (oracle base/USD ÷ reflector-fx or chainlink fiat/USD); success_count now reaches the trust floor and the corroborated release works unattended — verified live (XLM/EUR + XLM/GBP two-source medians on first tick). Original note kept below for history.
...
- **Hardware: ⛔ NOT A LEVER — do not propose it.** All 4× 7.68 TB NVMes are fully allocated (**0 GB unpartitioned, no spare drive**), and a 5th NVMe / raidz-expansion is **ruled out by a standing operator constraint**: *"R1 is NOT hardware-upgradeable. Fixed 4× 7.68 TB NVMe, no 5th drive, no raidz expansion. Never propose a drive upgrade."* (`production-readiness-remaining.md` §4; recorded verbatim in ADR-0027 as *"I cannot expand the capacity of this server."*) This line previously recommended a 5th NVMe as an `[OP]` action — that was wrong and is corrected (audit-2026-07-23 DOC-05). Capacity relief is **software-only** (galexie-archive → cold S3 ≈5.5 TiB, ZSTD recompression, tx_hash FixedString, ADR-0027 LCM cold-tier) plus horizontal growth onto a **separate second server**.
