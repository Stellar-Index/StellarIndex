...
- **NEXT: Phase D — comprehensive backfill** (now fully unblocked: capacity solved + v0.17.0 deployed). Archive-walk the degraded ranges `[genesis→38M]`+`[54→63M]` (disk-safe per-partition, fresh state file), in-CH ordinal reproject `[38→54M]`+`[63M→tip]`, `ledger_entries_current` reproject, PG served-tier re-derives, then Phase E prove. ~1–2 wk, resumable. Let Phase A recompress finish first (contract_events, ~1–2h) then start.
