...
- **Phase:** A (Capacity). **Running now:** finishing `ledger_entry_changes` recompress — **8 of 16 done**; the last 8 (`40 41 44 45 47 51 52 53`) had **stalled** at LZ4 (they need a >150 GiB single-part merge that the 150 GiB ceiling blocked), so a **one-at-a-time finish-driver** (`/usr/local/sbin/finish-recompress.sh`, log `/var/log/recompress-lec-finish.log`) is doing them with the ceiling raised to 500 GiB; watcher active (bg). Live healthy (3 svc up, tip advancing), pool ~91% (free ~0.95 TiB). Net-positive on space but tight — 4 concurrent merges possible; watcher exits at <350 GiB free for manual intervention. **The finish-driver auto-reverts the temp settings** (ceiling→150 GiB, `old_parts_lifetime`→480) on completion.
