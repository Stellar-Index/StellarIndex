...
- **Lake dedup chain — 🔵 RUNNING** (systemd `lake-dedup-chain`, log `/var/log/lake-dedup-chain.log`): per-partition `OPTIMIZE FINAL` removing the ~50% duplicate rows. ledgers/transactions/operations/operation_results ✅ done; `contract_events` in progress; smaller tables after. Pool **521 G → 2.3 T+ free** so far. Ingest healthy throughout (lag <10s). Serving-plane alert flaps (latency, data_source_stale, phoenix/soroswap insert-stale) dispositioned as merge-window transients — re-check after chain completes.
