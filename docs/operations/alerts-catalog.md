...
- **Severity** — the rule's own `labels.severity`, which is what
  ...
  | `ticket` | 166 | `receiver: chat-default` | Discord **#stellarindex-alerts**, `repeat_interval` 24 h. |
...
| `stellarindex_projector_row_dropped_permanent` | `increase(stellarindex_projector_events_decoded_total{outcome="sink_permanent"}[15m])` | > 0, `for: 0m` — the sink rejected a row's VALUES (SQLSTATE class 22/23, or a canonical value-shape check before the statement ran) and the cursor advanced past it; a child born non-zero counts. Sister of `row_quarantined`, which had a rule while this arm had none. Read the RATE: a burst across one source is the GLOBAL form of the same SQLSTATE (a migration whose NOT NULL / CHECK the live rows violate) | ticket | [projector-row-quarantined](runbooks/projector-row-quarantined.md) |
