...
- **Severity** — the rule's own `labels.severity`, which is what
  ...
  | `ticket` | 191 | `receiver: chat-default` | Discord **#stellarindex-alerts**, `repeat_interval` 24 h. |
...
| `stellarindex_decimals_guard_sweep_stale` | `(time() - stellarindex_decimals_guard_sweep_last_success_unix{job="stellarindex-aggregator"}) > 2700 and ...{job="stellarindex-aggregator"} > 0` | for ≥ 15 min (three sweep intervals — the decimals-guard sweep wedged, or never armed within 60 min of aggregator start: the gauge is seeded with the start time before the lake dial and Backfill; a lake-less aggregator keeps it at 0 and is silent, since the guard is disabled by config; the offender counters above cannot be trusted while this is stale) | ticket | [dex-nonstandard-decimals](runbooks/dex-nonstandard-decimals.md) |
