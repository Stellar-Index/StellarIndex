...
- **Severity** — the rule's own `labels.severity`, which is what
  ...
  | `ticket` | 182 | `receiver: chat-default` | Discord **#stellarindex-alerts**, `repeat_interval` 24 h. |
...
| `stellarindex_decimals_guard_sweep_stale` | `(time() - stellarindex_decimals_guard_sweep_last_success_unix{job="stellarindex_aggregator"}) > 2700` (r1: `job="stellarindex-aggregator"`; each tree names its own scrape job) | for ≥ 15 min (three sweep intervals — the decimals-guard sweep wedged or never armed; the offender counters above cannot be trusted while this is stale) | ticket | [dex-nonstandard-decimals](runbooks/dex-nonstandard-decimals.md) |
