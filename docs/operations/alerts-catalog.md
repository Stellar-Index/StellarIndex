...
| `stellarindex_projector_replay_stalled` | `stellarindex_projector_replay_window_active == 1` AND `max_over_time(stellarindex_projector_lag_ledgers[15m]) <= stellarindex_projector_lag_ledgers` | replay window open and lag has not fallen for 15 min, for 5 min (the operator's rewind has stopped advancing — the served-row deficit it was started to repair is still open) | ticket | [projector-replay](runbooks/projector-replay.md) |
