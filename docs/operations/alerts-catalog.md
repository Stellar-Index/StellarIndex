...
| `stellarindex_ingestion_trade_buffer_drop` | `increase(stellarindex_source_insert_errors_total{kind="dropped"}[15m])` | > 0, `for: 0m` — ONE permanently dropped trade tickets at once and stays up for the 15 min window; a child born non-zero counts (was `for: 15m`, which a single drop could never satisfy) | ticket | [trade-insert-backpressure](runbooks/trade-insert-backpressure.md) |
...
| `stellarindex_ops_job_no_progress` | `changes(stellarindex_ops_job_progress_total[30m])` + `changes(stellarindex_ops_job_progress_bytes_total[30m])` + `_running`, joined `on (ops_job, instance, pid)` | running==1 and BOTH zero units completed and zero storage observed to move in 30 min, for 15 min (the ops job is HUNG) | ticket | [ops-job-stalled](runbooks/ops-job-stalled.md) |
| `stellarindex_ops_job_run_failed` | `stellarindex_ops_job_last_exit_ok == 0` | the most recent run of an ops job ended with a non-zero exit, `for: 0m` (no `running` guard — this is the terminal state a finished run left) | ticket | [ops-job-stalled](runbooks/ops-job-stalled.md) |
...
| `stellarindex_ch_schema_drift_stale` | `time() - stellarindex_ch_schema_drift_last_run_unix` (or `absent_over_time(...[36h])` — never / every-run-failed) | > 36 h, or series absent 36 h, for ≥ 30 min | ticket | [ch-schema-restore](runbooks/ch-schema-restore.md) |
...
| `stellarindex_curated_rwa_published_stale` | `time() - stellarindex_curated_rwa_sync_executed_at_unix` while that gauge is non-zero — the sync runs but the curator's public total query has stopped re-executing | > 72 h for ≥ 1 h | ticket | [curated-rwa-sync-stale](runbooks/curated-rwa-sync-stale.md) |
...
| Name | Metric | Condition | Severity | Runbook |
| ---- | ------ | --------- | -------- | ------- |
| `stellarindex_ledgerstream_tier_both_missing` | `stellarindex_ledgerstream_tier_read_total{outcome="both_missing"}` | `increase(...[5m]) > 0`, `for: 0m` — ONE both-tiers miss pages at once (was `for: 5m`, which a single miss could never satisfy) | page | [ledgerstream-tier-both-missing](runbooks/ledgerstream-tier-both-missing.md) |
...
| `stellarindex_divergence_reference_failing` | `sum by (reference) (rate(stellarindex_divergence_reference_total{outcome!~"ok\|asset_unsupported\|too_stale_to_compare"}[15m])) > 0.5 * sum by (reference) (rate(...{outcome!~"asset_unsupported\|too_stale_to_compare"}[15m]))` | sustained 30 min (one reference failing most lookups — every pass still counts `ok`, so the pass-level rules cannot see it) | ticket | [divergence-no-reference](runbooks/divergence-no-reference.md) |
| `stellarindex_divergence_pair_below_quorum` | `max by (pair) (max_over_time(stellarindex_divergence_pair_quorum_met[30m])) == 0` | sustained 30 min (every refresh of the pair for an hour had fewer responders than `min_sources_for_warning`; its warning is carried forward, not evaluated) | ticket | [divergence-no-reference](runbooks/divergence-no-reference.md) |
...
| `stellarindex_aggregator_outlier_volume_trim_fraction` | `1 − window_base_volume{stage="outlier"} / window_base_volume{stage="class"}` on every window, class volume > 0 | > 0.2 for > 15 min | ticket | [aggregator-outlier-storm](runbooks/aggregator-outlier-storm.md) |
...
| `stellarindex_usage_rollup_failing` | `rate(stellarindex_usage_rollup_sweeps_total{outcome=~"scan_error\|sink_error"}[15m])` | > 0 for ≥ 30 min | ticket | [usage-rollup-failing](runbooks/usage-rollup-failing.md) |
...
| `stellarindex_sdex_orderbook_maintain_failing` | `rate(stellarindex_sdex_orderbook_maintain_total{outcome=~"load_error\|advance_error\|verify_error"}[15m])` | > 0 for ≥ 30 min (load_error = endpoint stuck warming; advance_error = book drifting from tip; verify_error = quarantine not draining, offers withheld) | ticket | [sdex-orderbook-maintain-failing](runbooks/sdex-orderbook-maintain-failing.md) |
| `stellarindex_sdex_orderbook_crossed_book` | `max(stellarindex_sdex_orderbook_crossed_pairs)` | > 0 for ≥ 30 min (a served pair has best bid > best ask — phantom offers) | ticket | [sdex-orderbook-maintain-failing](runbooks/sdex-orderbook-maintain-failing.md) |
| `stellarindex_ch_schema_probe_absent` | `stellarindex_ch_schema_probe_present` (per instance, per `probe`) | == 0 for ≥ 30 min (the lake answered "absent" or "empty" for an explorer schema probe; an absent answer before the process ever saw the object latches until restart, so the surface stays on its fallback) | ticket | [ch-schema-probe-absent](runbooks/ch-schema-probe-absent.md) |
| `stellarindex_protocol_events_rollup_failing` | `rate(stellarindex_protocol_events_rollup_sweeps_total{outcome="refresh_error"}[15m])` | > 0 for ≥ 30 min | ticket | [protocol-events-rollup-failing](runbooks/protocol-events-rollup-failing.md) |
| `stellarindex_asset_volume_rollup_failing` | `rate(stellarindex_asset_volume_rollup_sweeps_total{outcome="refresh_error"}[15m])` | > 0 for ≥ 30 min | ticket | [asset-volume-rollup-failing](runbooks/asset-volume-rollup-failing.md) |
| `stellarindex_nonstandard_decimals_correction_failing` | `increase(stellarindex_nonstandard_decimals_cache_refresh_failures_total[15m]) > 0 or increase(stellarindex_price_serve_declined_nonstandard_decimals_total[15m]) > 0 or increase(stellarindex_nonstandard_decimals_lockstep_mismatch_total[15m]) > 0`, plus an `unless ... offset 15m` arm per counter | > 0 for ≥ 5 min. The `offset` arms catch a counter whose first increment lands before its first scrape — `increase()` reads a flat range as 0. The lockstep arm fires per `{site, asset}` child: `guard_reconcile` when the aggregator repaired a `nonstandard_decimals_assets` row that disagreed with the lake, `asset_detail` when `GET /v1/assets/{id}` refused a market cap because the two decimals resolvers disagreed at request time | ticket | [dex-nonstandard-decimals](runbooks/dex-nonstandard-decimals.md) |
| `stellarindex_decimals_guard_sweep_stale` | `(time() - stellarindex_decimals_guard_sweep_last_success_unix{job="stellarindex_aggregator"}) > 2700 and ...{job="stellarindex_aggregator"} > 0` (r1: `job="stellarindex-aggregator"`; each tree names its own scrape job) | for ≥ 15 min (three sweep intervals — the decimals-guard sweep wedged, or never armed within 60 min of aggregator start: the gauge is seeded with the start time before the lake dial and Backfill; a lake-less aggregator keeps it at 0 and is silent, since the guard is disabled by config; the offender counters above cannot be trusted while this is stale) | ticket | [dex-nonstandard-decimals](runbooks/dex-nonstandard-decimals.md) |
...
| `stellarindex_price_alert_eval_no_successful_sweeps` | `rate(...{outcome="ok"}[5m]) == 0 and rate(...{outcome="partial_error"}[5m]) > 0` | sustained 30 min (every sweep hits an eval error, none go clean — the `list_error` alert above cannot see this shape since the list read itself succeeds) | ticket | [price-alert-eval-failing](runbooks/price-alert-eval-failing.md) |
| `stellarindex_price_alert_evaluations_failing` | `sum(rate(stellarindex_price_alert_evaluated_total{outcome=~"error\|timeout"}[15m])) > 0.5 * sum(rate(stellarindex_price_alert_evaluated_total[15m]))` | sustained 30 min (most per-alert evaluations fail — `partial_error` alone cannot tell one bad alert from all of them) | ticket | [price-alert-eval-failing](runbooks/price-alert-eval-failing.md) |
| `stellarindex_price_alert_sweep_stale` | `(time() - stellarindex_price_alert_last_sweep_unix) > 900 and stellarindex_price_alert_last_sweep_unix > 0` | for ≥ 5 min (no sweep completed in 15+ min; a wedged sweep emits no outcome sample, so the `rate()` rules read no data) | ticket | [price-alert-eval-failing](runbooks/price-alert-eval-failing.md) |
...
| Name | Metric | Condition | Severity | Runbook |
| ---- | ------ | --------- | -------- | ------- |
| `stellarindex_prometheus_scrape_failing` | `up{job=~"api\|indexer\|aggregator"}` | == 0 for any target > 2 min | ticket | [scrape-failing](runbooks/scrape-failing.md) |
...
| `stellarindex_metrics_registry_absent` | `stellarindex_metrics_registry_present{component}` | == 0 for 15 min (component running Registry-less → its metrics + dependent alerts are dead) | ticket | [metrics-registry-absent](runbooks/metrics-registry-absent.md) |
...
**#485 is settled (2026-09-08).** The 11 rules this register had
tagged `needs-delivery` were promoted to `severity: ticket` in both
rule trees, so they now route to `chat-default` and leave this
register; what remains is 10 rules that are silent *on purpose*. The
promotion was made against evidence rather than preference: none of
the 11 was firing at the time, so it added no immediate noise, and
10 of them carried no in-code rationale for being `informational` at
all — the "inherited from a copied YAML block" case this register was
written to expose. The 11th,
`stellarindex_timescale_job_failures_climbing`, had a comment arguing
it should stay ("the CAGG-staleness ticket above is the real
customer-impact signal") that its **own description** contradicts
("the caggs may look fresh and no staleness ticket will fire"); the
description matches the incident the rule was written for, so the
comment was the thing that was wrong, and it has been corrected in
place. A rule authored specifically to expose a silent failure mode,
