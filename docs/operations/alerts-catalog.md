...
| `stellarindex_decimals_guard_sweep_stale` | `(time() - stellarindex_decimals_guard_sweep_last_success_unix{job="stellarindex-aggregator"}) > 2700` | for ≥ 15 min (three sweep intervals — the decimals-guard sweep wedged or never armed; the offender counters above cannot be trusted while this is stale) | ticket | [dex-nonstandard-decimals](runbooks/dex-nonstandard-decimals.md) |
