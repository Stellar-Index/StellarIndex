...
| 1.14 | **The fiat point path is not alias-complete on the quote leg.** Single-bar `/v1/ohlc`, `/v1/vwap` and `/v1/twap` read each USD-pegged constituent's raw trades in its literal spelling (`TradesInRange` takes the pair it is given), so a declared peg's SAC wrapper — where every Soroban pool's USD leg lives — is never read, and an asset whose only USD depth is such a pool 404s. The series arm has the same blindness and its own row (1.15); `/v1/chart` does NOT, because its proxy walk already enumerates every canonical form of each peg (`usdPegProxyQuotes`'s classic pass then SAC pass), so that asset charts today and quotes nowhere else. Close this with whatever rule 1.15 settles on, in one shape for both paths, and pin point/series parity for the pool-only case — the two answering the same question from different populations is the C1-024 divergence this path was rebuilt to remove. | agent |
