...
| 1.7 | **Test nets are a release and three migrations behind again** — `v0.57.0`/schema 150 against r1's `v0.58.0`/schema 153, ONE DAY after being caught up. Migrations 0151-0153 have run only in production. The drift has no automatic prevention, and that is the actual finding. **PREVENTION BUILT (2026-09-04); mark DONE after the first scheduled run is observed.** The drift itself is closed — an operator read live `/v1/version` by hand on 2026-09-04 and got `v0.59.1` from r1, `api.testnet.stellarindex.io` and `api.futurenet.stellarindex.io` alike; that is an operator reading, NOT a run of the new check, which has not fired on its schedule yet, and it is why this row stays open — and what is new is that the next one cannot stay invisible: (1) [`fleet-release-drift.yml`](../../.github/workflows/fleet-release-drift.yml) compares the three served `/v1/version` values daily, with a 24h grace measured per test net from the OLDEST r1 release it lacks — never from r1's newest tag: measured that way, the 2026-09-03 cascade (`v0.58.0` 01:26Z, `v0.59.1` 07:30Z) would have kept the 2026-09-04 07:25Z run quiet with both test nets 29h behind `v0.58.0`, and every release under 24h apart would have re-opened the window — opens/updates a `fleet-release-drift` issue carrying the exact `deploy.yml` dispatches, fails the run so the scheduled-failure mail arrives, and closes the issue only once every test net serves r1's release; an unreadable host is `DEGRADED`, never "in step". (2) `deploy.yml` gained a **Fleet release parity** step that, after every r1 deploy, lists both test nets against the tag just deployed and prints the catch-up commands (a warning — r1 goes first by design). `/v1/version` is a static handler, so it proves which release a host runs and nothing more; migrations are derived from the tags, exact on the default deploy path (`deploy-binary.yml` applies the tag's migrations before the swap) and blind to one override, `migrations_skip=true`, under which a host serves its tag's version over an older schema (its `/v1/readyz` answers 503 on the `schema` check; `/v1/healthz` and `/v1/version` keep answering). A Prometheus rule was the wrong tool here: r1 scrapes only itself and the VMs are NAT-only, so no series exists to compare and no codified scrape target could reach them. Self-test pins v0.57.0-vs-v0.59.1 RED (3 releases, 4 migrations), the same-day cascade RED on the real tag dates, the in-step fleet GREEN, a served "version" carrying newlines and a forged `verdict=in-step` line DEGRADED (every host byte is sanitised to one line and the workflow's multi-line output ends on a random delimiter), a pre-release on r1 fail-closed (drift at once, no grace, and only an IDENTICAL version string in step under it, since every pre-release of a base compares as one version — `v0.60.0-rc.2` on r1 beside `v0.60.0-rc.1` on a test net is drift, not a quiet run that closes the ticket), a run with no follower to compare fail-closed rather than in-step, and a version's own bytes bounded by the pattern (a host serving `v0.08.0` or 10,000 characters of suffix gets a clean verdict, not a dead run or an unbounded issue body): `bash scripts/ci/check-fleet-release-drift-test.sh`. Not built: auto-dispatching the test-net deploys after r1 (the continuous test-tier deploy in `cicd-fleet-model.md` §2) — a policy change over the "a human decides when each region picks a release up" rule in `deploy.yml`; and observing the schema through `/v1/readyz` per host, which would close the `migrations_skip` blind spot. | agent |
...
| 1.14 | **The fiat point path is not alias-complete on the quote leg.** **STILL OPEN, and now GATED ON 1.15 rather than the other way round (2026-09-05).** Single-bar `/v1/ohlc`, `/v1/vwap` and `/v1/twap` read each USD-pegged constituent's raw trades in its literal spelling, so a declared peg's SAC wrapper — where every Soroban pool's USD leg lives — is never read, and an asset whose only USD depth is such a pool 404s. `/v1/chart` does NOT, because its proxy walk enumerates every canonical form of each peg. **The mechanism is settled and is NOT the direction fold** ([aggregate-alias-folding](../architecture/aggregate-alias-folding.md) §7): folding a DIRECTION adds no market and can only be got wrong arithmetically; folding an ALIAS adds a different market, so it is a liquidity decision. What the alias rows need is the merge/last split already shipped for `/v1/price/tip` (`tipMergePairs`) — established spellings merge, SAC forms are read only when every established read has missed — combined with `usdPegProxyQuotes`'s classic-pass-then-SAC-pass ordering across ALL families. That fits the point path completely: one window, one aggregate, so 1.15's requirement (b) about suppressing a family's later BUCKETS has no analogue here. **What blocks it is not the mechanism.** `usdPeggedConstituents` feeds the point path and the series from one list, and `TestFiatVWAPPointMatchesSeries` / `TestFiatSingleBarOHLCMatchesSeriesExtremes` pin the two to one population by comparing price, both volumes and both extremes. Widening one side alone turns that pin red — and the pin is right, since two surfaces answering one question from two populations is the C1-024 defect this path was rebuilt to remove. Closing 1.14 alone would trade a 404 for a fresh divergence. Land 1.15 first, then this closes with it in one shape. **(2026-09-05: the cross-scale defect 1.15 was blocked on is fixed, so 1.15 is buildable and this row's dependency is now only on 1.15 itself. Nothing about this row's own mechanism changed — it still needs `tipMergePairs`'s merge/last split combined with `usdPegProxyQuotes`'s classic-then-SAC ordering, landed together with 1.15 so the two surfaces keep one population.)** The `distinctMarkets` dedupe that shipped with 1.16 removes the double-count this widening would otherwise introduce, since it would put a peg's SAC form in the quote set where a base alias already sits. | agent |
| 1.15 | **A fiat-quoted `/v1/ohlc` series cannot reach a SAC-quoted Soroban pool.** The combine reads each USD-pegged constituent under the one quote spelling the peg expansion names it in — a declared peg in its classic form, an abstract backer, or the fiat — and `Store.OHLCSeries` takes that spelling literally, so a pool stored as `<AQUA SAC>/<USDC SAC>` is out of reach while the SDEX book at `AQUA/USDC-GA5Z…` is not. An asset whose only USD depth is such a pool therefore serves `intervals: []`; its coverage floor is now scoped to the same constituent pairs, so the empty series carries no floor rather than a false "quiet since the pool's first bucket", which describes the gap without closing it. Closing it needs three things together, and two candidate shapes were measured and rejected. Reading the pool as a constituent of its own folds it into the book's bucket: on the shipped single-peg config, a `crypto:XLM/fiat:USD` book of n=100 over 6,000,000 units beside a two-print `<XLM SAC>/<USDC SAC>` pool serves n=102, v_base=6,000,020, high 0.50, low 0.01 — two prints setting the bar's extremes against six million units of book volume, because the per-bucket combine takes max and min across constituents. Gating the pool on its OWN quote family being empty leaves that fault intact ACROSS families (the pool's family is empty, so it is read and summed into another family's bucket) and adds a second: with the book holding day 1 and the pool days 2–30, one bar is served and 29 days of real prints are suppressed and reported as quiet. So: (a) a first-hit rule across ALL established spellings of ALL families, never per family; (b) a decision on whether suppressing a family's later buckets is acceptable, and if not, what replaces first-hit; (c) a guarantee that a thin pool never sets a bar's high, low, count or volume beside book data. Whatever lands must keep `canonical.AssetAliases`'s SAC-last ordering, keep the series consistent with the live aggregator's own source set (which does not read the pool), and pin the served bars for both measured shapes. Note the cross-surface disagreement this leaves standing: `/v1/chart`'s proxy walk DOES reach the wrapper (`usdPegProxyQuotes` runs a classic pass then a SAC pass), so a pool-only asset charts while its OHLC series is empty — settle 1.14 and 1.15 in one shape so all three surfaces answer from one population. **REVIEWED 2026-09-05, NOT ATTEMPTED — deliberately left open, with the blocker now named.** The record was checked before touching this: there is no lineage of five attempts at this row. It was CREATED in one commit with two candidate shapes measured and rejected in that commit's working tree, and has never been attempted. The six-commit lineage that does exist is on the surrounding class — the fiat combine and thin prints setting its extremes — and every one of those changes fixed its predecessor's defect and introduced its own, which is the pattern a sixth pass would repeat. **New since the two shapes were measured:** (a) and (c) have both shipped on sibling surfaces (`usdPegProxyQuotes`'s classic-then-SAC pass across all families; `tipMergePairs`'s merge/last split, which is exactly "a merge has no first, so give it a last"); migration 0115's sub-cent notional floor on the CAGG extremes is materialised via 0147, so (c) is now only about a legitimate-notional thin pool rather than dust; and row 1.16's fold gives a costed precedent for widening a pair-bound read with no plan regression. ~~**What is missing is mechanical and disqualifying.**~~ **THE BLOCKER IS FIXED (2026-09-05) — this row is now buildable and is still NOT built.** The blocker was recorded as: `ohlcSeriesFiatCombined` combines CAGG BARS, a bar has no `source` column, so `NormalizeAmountScale` cannot run on this path. **Both halves of that were wrong.** A CAGG bar has carried `array_agg(DISTINCT source) AS sources` since migration 0002 and still does under 0147 — `Store.OHLCSeries` simply never selected it, so the column was missing from the Go struct, not from the database. No migration and no re-materialisation was needed. And the defect was **LIVE, not latent**: it was filed on the assumption that today's constituents share a scale, and on r1 they do not. The `native/fiat:USD` combine already merges `native/<USDC classic>` (sdex, 7dp) with `crypto:XLM/crypto:USDT` (binance, 8dp) and `crypto:XLM/fiat:USD` (bitstamp/coinbase/kraken, 8dp) in the SAME buckets — measured on the 02:00Z 1h bar, the sdex leg's 135,713.79 XLM entered the combine as 13,571.38, the served `v_base` understated the market by **3.41%**, and that leg carried 0.39% of the open/close weight against a real 3.79%. Since the point path has normalised the identical constituent set since CS-040, point and series were weighting one population two ways: a live C1-024 violation that the pinned parity tests could not see, because every trade in their fixture is stamped `sdex`. **What shipped:** both bar readers carry `sources` folded across BOTH stored directions (exact, not clever — the CAGG groups on `(bucket, base_asset, quote_asset)` so a bucket holds at most one row per direction), and the combine lifts every bar to the maximum scale in the response, an exact integer multiply by `10^(max−scale) ≥ 1`, no division (ADR-0003). Single-scale responses are byte-identical, so every non-fiat quote is untouched and both readers return every numeric column unchanged on live data. `EXPLAIN` on r1, plan only: `OHLCSeries` 29911 → 30076 (**1.005x**), `OHLCSeriesReBucketed` 1h → 4h 31003 → 38892 (1.25x). **What this row now needs**, unchanged and undiminished: (a) the first-hit rule across ALL established spellings of ALL families, (b) the decision on suppressing a family's later buckets, (c) the guarantee that a thin pool never sets a bar's high, low, count or volume beside book data — shape A's `n=102 / v_base=6,000,020` is a fault of the per-bucket max/min and sum, and the scale fix does not touch it. What HAS changed is that a widened set can no longer mix scales silently: a 7dp pool bar beside 8dp exchange bars is now lifted rather than summed raw, so shape A's volume figure is no longer additionally understated. Full reasoning in [aggregate-alias-folding](../architecture/aggregate-alias-folding.md) §7.4. | agent |
| 1.16 | ~~**Four surfaces still read ONE stored orientation of a market — and `/v1/history` no longer does, so they now DISAGREE.**~~ **MECHANICAL HALF DONE (2026-09-05); the aggregate half is open and this row does NOT close it.** `/v1/observations`, its SSE stream and `/v1/price`'s last-trade fallback now serve a market whichever way round it is asked for, so they agree with `/v1/history` about whether one exists. **The row's own inventory was wrong on both readers and is corrected here:** `/v1/observations` reads `Store.LatestTradePerSource` (not `Store.TradesInRange`), and `Store.LatestTradesForPair` is read by `storePriceReader.LatestPrice`'s last-trade arm — the `/v1/price` family — not by the status page, which reaches these rows only THROUGH `/v1/observations`. `Store.TradesInRange` is read by `/v1/vwap`, `/v1/twap`, single-bar `/v1/ohlc` **and `/v1/price/tip`** plus the aggregator orchestrator: a fifth serving surface this row did not name, which the aggregate half must cover. **What shipped:** both mechanical readers select BOTH stored directions as two unioned arms — each arm is the query it used to be, so each keeps its index-ordered scan (`trades_pair_source_ts_idx` / `trades_pair_ts_idx`) and its early stop, where a single OR'd predicate would have had to bitmap both directions and sort them on a read whose worst case is already a full-history walk for an empty pair. Flipped rows are re-expressed per ROW on the row's own `base_asset`, by SWAPPING the two legs and the two smallest-unit amounts — exact at any magnitude, no division performed anywhere, so a zero amount cannot poison a row — which is the same rule `/v1/history` and `Store.OHLCSeries`'s `norm` CTE apply. **`Store.LatestTradePerSource` needed a fold rule, not merely a wider read:** one source that traded the market both ways round now arrives twice and the surface is one row per source, so the two are folded to whichever trade came later, compared on `(ts, ledger, tx_hash, op_index)` — a TOTAL order within one source (the trades primary key is those four plus `source`) and the same four `/v1/history` orders its page on. `source` is never compared, so no Go ordering has to agree with the database's collation, and the selected row is the same whichever way round the market is asked for. **Cost, measured not asserted:** EXPLAIN on r1 (plan only) puts the per-source read at 4243 → 8487 with every arm keeping its per-chunk SkipScan over the compressed index, and the last-trade read at an early-stopping `Limit` 1.67 → 2.26 — exactly twice, well inside the unchanged 8s ceilings. **No probe needed widening:** neither surface carries a coverage floor, and the `/v1/price` stablecoin proxy's gate `RecentClosedVWAP1mExists` already spanned both directions, so the read has caught up with its probe rather than outrun it. Spec 1.24.0 → **1.25.0** (description only); reference docs, Postman collection and explorer types regenerated. **AGGREGATE HALF DONE (2026-09-05) — this row is now closed.** `Store.TradesInRange` reads both stored directions as two limited arms and re-expresses per ROW, so `/v1/vwap`, `/v1/twap`, single-bar `/v1/ohlc`, `/v1/price/tip` and the orchestrator compute over the whole market. **Measured, not asserted:** one hour of `native/USDC-GA5Z…` on r1 holds 2957 rows one way round and 2794 the other, so the served window was 51.4% of the market — and a biased 51.4%, the sell side, since the decoder sets `base = soldAsset`. The VWAP moved -0.0016%; the EXTREMES did not survive — the served high understated the hour's true high by 0.65% (0.1806435916 vs 0.1818181818) and the served low was 0.12% too high, on a bar that never looked empty. It also closed a live C1-024 violation the parity tests cannot see, the point path reading raw trades one way while the series reads a CAGG that folds. **The design question this row deferred is answered and written down** ([aggregate-alias-folding](../architecture/aggregate-alias-folding.md)): every aggregate here is defined on the two integer LEG AMOUNTS and never on a stored price, so the leg swap re-weights the mean at the same instant as it inverts the price — the weight IS the row's base leg, and a flipped row's base leg in the requested orientation is its stored quote amount. No division, no second re-weighting step. Re-expression happens BEFORE any comparison because an inverted price turns a maximum into a minimum: appending flipped rows unre-expressed reports a **high of 10.0** where the market's high is 0.25, a fully populated and entirely wrong bar, pinned red-on-unfixed. Two fixture-free properties are pinned too — `VWAP(A/B) x VWAP(B/A) == 1` exactly, which only a re-weighted fold satisfies, and `low < VWAP < high`. Truncation still keeps the newest rows and the orchestrator's detector still fires (correctly, more often). **Cost from EXPLAIN on r1, plan only:** a `Merge Append` over two independently limited arms, every arm keeping its `trades_pair_ts_idx` scan and its per-chunk `ColumnarScan` — 1 h populated 25.75 → 44.14 (1.72x), 24 h populated 1169.12 → 1191.84 (**1.02x**), empty pair over full history 147.06 → 184.61 (1.26x), all inside the unchanged 8s ceilings. **The class guard shipped with it**, widened from `FROM prices_*` to the `trades` hypertable, with exactly ONE exemption that does not say a read is blind: `TradesInRangeAfter` is folded by its caller under a keyset cursor, which a union cannot carry. The widened scan found a third pair-bound trades read the row did not name — `FXQuoteAtOrBefore`'s legacy fallback on the triangulation money path — and since it serves a price it is not exemptible by that guard's own rule, so it is folded rather than excused (dormant: zero rows on r1 across both FX sources over 400 days). One consequence worth recording: folding direction in the store makes a pair and its flip ONE read, and `/v1/price/tip?asset=native&quote=crypto:XLM` really does emit both into one merged VWAP, so the two merging sets now deduplicate by market and keep the first spelling. No spec change — the fold is invisible on the wire. |
...
2026-09-04** ([artefact](../audit/d7-thin-pool-third-alias-vwap-review-2026-09-04.md));
...
| C4-012/13 third-alias thin-pool VWAP surface | **DONE 2026-09-04** — [artefact](../audit/d7-thin-pool-third-alias-vwap-review-2026-09-04.md); `/v1/price/tip` fixed, residual R1 is the remaining decision |
