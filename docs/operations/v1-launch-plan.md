...
| 1.13 | **`/v1/history` reads ONE stored orientation.** `Store.TradesInRangeAfter` never flips a market, so a market the decoder recorded only as USDC/AQUA answers `base=AQUA&quote=USDC` with an empty page every time, while `/v1/ohlc` and `/v1/chart` on the same market fold both directions. The page's coverage floor now spans the orientation the read spans, so it no longer claims "quiet since" over rows the page cannot return — that describes the gap without closing it. Decide whether the page folds the reverse orientation (and how a flipped row renders its price and amounts) or documents the single orientation as the contract; either way, pin it. | agent |
