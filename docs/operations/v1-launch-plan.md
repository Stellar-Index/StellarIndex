...
| 1.16 | **Four surfaces still read ONE stored orientation of a market — and `/v1/history` no longer does, so they now DISAGREE.** Row 1.13 closed the raw-trade page and nothing else. `Store.TradesInRange` (`internal/storage/timescale/trades.go`) and `Store.LatestTradesForPair` both bind `base_asset = $1 AND quote_asset = $2` with no flipped arm, so `/v1/observations`, `/v1/vwap`, `/v1/twap` and the single-bar `/v1/ohlc` keep exactly the blindness 1.13 was opened for: a market the decoder recorded only as USDC/AQUA is invisible to all four under `base=AQUA&quote=USDC`. The cross-surface disagreement is NEW and was created by closing 1.13 — the same request now returns trades from `/v1/history` and nothing from `/v1/observations`, which is the shape of defect this row's predecessor existed to remove. Note also that the class guard `TestCAGGPairReadsFoldBothDirections` scopes itself to `FROM prices_*` and structurally cannot see any read of the raw `trades` hypertable, which is why the class survived two remediations there; extending it to `FROM trades` is part of this row. `/v1/observations` and `/v1/history` are the mechanical cases (per-trade rows, so the same integer-swap re-expression applies). `/v1/vwap` / `/v1/twap` / single-bar `/v1/ohlc` are NOT mechanical: they aggregate, so folding a flipped row into a mean or an extreme is the design question rows 1.14/1.15 are already circling, and it should be settled with them in one shape rather than three. | agent |
