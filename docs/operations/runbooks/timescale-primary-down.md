...
- Alert `stellarindex_timescale_primary_down` fires — `up{role="primary"}` = 0 for > 30 s.
- API latency spikes on write endpoints (`POST /v1/account/keys` etc.) — reads from Redis hot path still work.
- `stellarindex_ingestion_cursor_stuck` lights up across all sources within ~60 s.
...
```sh
curl -fsS https://api.stellarindex.io/v1/readyz | jq .checks.postgres
...
# `/service/<scope>/`, not `/stellarindex/`. Earlier prose
# named both wrong.

# And directly poke the primary's psql:
PGCONNECT_TIMEOUT=3 psql -h db-primary.internal -U stellarindex \
  -d stellarindex -c 'SELECT pg_is_in_recovery();'
...
- [ ] Indexer logs resume inserting trades (`journalctl -u stellarindex-indexer -f`).
...
patronictl -c /etc/patroni/patroni.yml failover stellarindex --candidate db-replica-sync.internal
...
- Promote the async replica (in R3 per multi-region-topology.md §5.3):
  ```sh
  patronictl failover stellarindex --candidate db-replica-async.internal
  ```
...
- On return, reconcile cursor state: the indexer's idempotent
  ...
  Check `stellarindex_ingestion_cursor_stuck` clears within 2 min.
...
1. **Disk full** — WAL couldn't write; Postgres halted writes. Catches: `stellarindex_timescale_disk_warning` fires BEFORE this one usually. If it didn't, tune thresholds.
...
3. **Kernel / ZFS issue** — NVMe drive dropped, ZFS pool degraded. Catches: `stellarindex_zfs_pool_degraded` fires first.
