...
| `ch_drop_guard_live` | ClickHouse destructive-DDL size guard (2026-08-29 r1 audit: both limits ran at 1 TiB from a hand-written `config.d` file, so every lake table was droppable in one statement). FAIL = live `max_table_size_to_drop` or `max_partition_size_to_drop` in `system.server_settings` is `0`, above the pinned 50 GB (`53687091200`), missing, or ClickHouse did not answer on `:8123` | find the carrier: `sudo grep -rn 'size_to_drop' /etc/clickhouse-server/config.xml /etc/clickhouse-server/config.d/`; remove any file other than `si-drop-guard.xml`, then re-apply `--tags clickhouse-drop-guard` (hot-reloads, no restart). A planned big drop uses the `force_drop_table` flag, never a raised limit — [clickhouse-destructive-ddl.md](../clickhouse-destructive-ddl.md) |
| `tx_hash_index_parity` | `GET /v1/tx/{hash}` treats a `stellar.tx_hash_index` miss as an authoritative 404 (2026-07-30 audit); the index is MV-maintained and was parity-verified genesis→tip only ONCE. This probe samples 500 recent tx hashes hourly and requires all to resolve in the index — a FAIL means the MV was dropped/recreated with a gap, or rows entered `stellar.transactions` without firing MVs (ATTACH-style load) | find the divergence window (compare per-ledger presence around the FAIL's sample hashes), then re-run the windowed backfill: `stellarindex-ops ch-txindex-backfill -ch-addr 127.0.0.1:9300 -from <window start> -to <window end> -window 5000000 -write` (idempotent — ReplacingMergeTree collapses re-inserts); verify `tx_hash_index_mv` exists |
