...
`stellarindex_aggregator_fx_snap_fallback_total{leg=...}`.
...
- `sum(rate(stellarindex_aggregator_fx_snap_fallback_total[15m])) /
  sum(rate(stellarindex_aggregator_triangulations_total{outcome="ok"}[15m])) > 0.5`
  for 30+ minutes.
...
| Very stale (>30 min) | Connector down | Both FX sources failing simultaneously | Check `stellarindex_source_events_total{source=~"polygon-forex|exchangeratesapi"}` — if zero, the connector(s) are dead |
...
- [ ] **Stale rows in hypertable but connector says it's running**:
      ...
      `stellarindex_source_events_total` increments but the table
