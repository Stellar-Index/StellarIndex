...
2. **Check the prewarm code.** Open `cmd/stellarindex-api/main.go`, function `prewarmOnce`. Find the call corresponding to the alerted op. Compare every argument against what the handler at `internal/api/v1/markets.go` passes.
