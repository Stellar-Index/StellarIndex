...
- 2026-09-22 — dropped the dangling PR citations for the Limit
  ...
  (`verify-lake`/`verify-contiguity` under-reporting), so the
  citation pointed a reader at the wrong thing rather than at
  nothing. Replaced with a plain description of the bug the
  citations meant to name.
