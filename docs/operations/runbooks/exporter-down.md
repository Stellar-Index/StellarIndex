...
# Runbook — `ratesengine_{redis,postgres,pgbackrest,minio}_exporter_down`

## At a glance

| Field | Value |
| ----- | ----- |
| Alert | `ratesengine_<exporter>_down` (one per exporter) |
| Severity | P1 (page) |
...
`ratesengine_redis_writes_blocked` alert watches
...
- **redis_exporter** — Debian unit `prometheus-redis-exporter`,
  ...
  Feeds `cache.yml` + `ratesengine_redis_writes_blocked`.
...
- **pgbackrest_exporter** — unit `pgbackrest_exporter`, port 9854.
  Feeds `ratesengine_timescale_backup_failed` +
  `ratesengine_timescale_backup_none_24h`.
