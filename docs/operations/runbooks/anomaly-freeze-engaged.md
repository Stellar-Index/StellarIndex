...
| Field | Value |
| ----- | ----- |
| Alerts | `stellarindex_anomaly_freeze_engaged` (P3) / `stellarindex_anomaly_freeze_sustained` (P1) |
...
- `stellarindex_anomaly_freeze_engaged{asset="..."}` gauge = 1
- `stellarindex_anomaly_z_score{asset="..."}` histogram shows a
  recent spike (5σ+)
- `stellarindex_anomaly_confidence{asset="..."}` gauge dropped
  sharply below 0.10
...
ssh r1 'curl -s "http://localhost:9090/api/v1/query?query=stellarindex_anomaly_freeze_engaged" | jq'
ssh r1 'stellarindex-ops list-cursors -config /etc/stellarindex.toml | grep <asset>'

# 2) What does our raw observation show right now?
curl -s "https://api.stellarindex.io/v1/observations?asset=<asset>&quote=fiat:USD" | jq
...
stellarindex-ops verify-decoders -config /etc/stellarindex.toml \
...
- [ ] **Step 2 — Decide: confirm or override.**
  ...
  **If overriding the freeze (legitimate market event):**
  ...
  stellarindex-ops anomaly unfreeze -asset <asset-id> \
...
- [ ] **Verification:** `stellarindex_anomaly_freeze_engaged{asset=<x>}` returns to 0 after auto-unfreeze or manual override.
