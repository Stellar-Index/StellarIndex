...
- `time() - max(timestamp(stellarindex_aggregator_supply_refresh_total{outcome="ok"})) > 30 * 60`
  for ≥ 5 min.
...
sudo systemctl status stellarindex-aggregator
...
sudo journalctl -u stellarindex-aggregator --since "1 hour ago" -n 200 | \
