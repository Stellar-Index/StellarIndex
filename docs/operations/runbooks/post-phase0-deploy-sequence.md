...
- **Command (comprehensive, 2026-07-18 decision):** `stellarindex-ops ch-backfill -from 2 -to <tip>` — **genesis→tip**, with the deployed **new** binary, **windowed** under `run-heavy-job.sh`, off-peak. One contiguous pass also fills the entry-change genesis-edge gap `[2 → 287,404]` (currently 0 rows) so the state-diff substrate is complete from genesis — no compromise for simplicity, never repeat this backfill. (Feasible: tx/op already reach ledger 3 from the same archive, so `galexie-archive` holds the genesis LCM.)
...
- **Tie range is PROVEN:** a full scan of `[287,404 → 37,999,997]` found **zero** same-ledger multi-changes; they begin in (38M, 40M]. So below 38M every key has ≤1 change/ledger — `intra_ledger_seq=0` is unambiguous there — and the *current-state reproject* windows only need `[38M → tip]`. The genesis→tip **re-derive** is still run in full (for complete state-diff coverage + uniform provenance); only the reproject narrows to the tie range.
