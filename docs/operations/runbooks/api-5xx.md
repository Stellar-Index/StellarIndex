...
| Field | Value |
| ----- | ----- |
| Direct-threshold alerts | `stellarindex_api_error_rate_high` (>1 % for 2 min) → P2<br>`stellarindex_api_error_rate_critical` (>5 % for 2 min) → P1 |
| SLO burn-rate alerts | `stellarindex_slo_availability_burn_{fast,medium,slow}` (per ADR-0009 multi-window pattern) — `deploy/monitoring/rules/slo.yml` |
...
promql 'topk(5, sum by (route, status) (rate(http_requests_total{job=~"stellarindex[_-]api",status=~"5.."}[5m])))'
...
for h in api-01 api-02 api-03; do
  echo -n "$h: "
  ssh root@$h "systemctl show stellarindex-api -p ActiveEnterTimestamp --value"
done
...
ssh root@api-01 "journalctl -u stellarindex-api --since '15 min ago' \
...
2. **Feature-flag deny in the binary** (if a flag exists for the
   endpoint): edit `/etc/stellarindex.toml`, ansible-push, then
   `systemctl restart stellarindex-api` host-by-host.
...
- `ssh root@api-XX "journalctl -u stellarindex-api --since '1h ago'"`
...
- `systemctl status stellarindex-api --no-pager` on each host —
