...
- **F-1347 (blend/phoenix/soroswap topic-only Matches pollution)** — these decoders match foreign contracts' events, but that ADDS rows (noise), it does not LOSE data, and the EVERY-event policy favors capturing more. Gating by contract registry would REDUCE coverage. Left as-is; the behavior is consistent with the project's capture-everything stance.
