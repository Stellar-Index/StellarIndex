...
| SEP-41 Soroban token identity + events | `internal/sources/sep41_supply/` decoder; `internal/canonical/discovery/` Sniffer + AsyncSink (`cmd/stellarindex-indexer/main.go:disp.SetDiscoverySink`) | ✅ |
...
| Auto-discovery of new SEP-41 contracts | `internal/canonical/discovery/Sniffer` + `discovered_assets` hypertable (`migrations/0006_create_discovered_assets.up.sql`) + `stellarindex_discovery_dropped_hits_total` backpressure metric | ✅ |
| `home_domain` → `stellar.toml` SEP-1 resolution | `internal/metadata/sep1.go` Resolver + `cache.go` Redis cache + `cmd/stellarindex-api/main.go:177` wiring | ✅ |
...
| **Chainlink** (HTTP cross-check, no on-chain Stellar) | `internal/divergence/chainlink/` reference; wired in `cmd/stellarindex-api/main.go::buildDivergenceReferences` | ✅ |
...
| CEX feeds (Binance, Coinbase, Kraken, Bitstamp) | `internal/sources/external/{binance,coinbase,kraken,bitstamp}/`; runner wires via `setSourceEnabled` (`cmd/stellarindex-indexer/main.go:233`) | ✅ |
...
| **CAGG `twap` column is NOT real TWAP** (arithmetic mean) | `migrations/0002` notes; `/v1/twap` ignores the column and computes from raw trades | ✅ (caveated; `cmd/stellarindex-aggregator/main.go` carries the warning) |
...
| **Indexer wiring of all 6 LCM observers** | `cmd/stellarindex-indexer/main.go::pipeline.{RegisterSupplyEntryDecoders,RegisterSupplyEventDecoders}` driven by `[supply.*]` config | ✅ |

### C2. `change_24h_pct`

| Aspect | Code evidence | Status |
|---|---|---|
| OpenAPI declares the field | `openapi/stellar-index.v1.yaml:1400` | — |
...
| API reference documentation | `make docs-api` regenerates `docs/reference/api/index.html` from `openapi/stellar-index.v1.yaml`; `.github/workflows/api-docs.yml` deploys to GitHub Pages | ✅ |
...
`status.stellarindex.io` — no `deploy/` artefacts, no DNS, no
...
`cmd/stellarindex-api/main.go::main` constructs `streaming.NewHub(0)`
...
per `docs/operations/public-flip.md`; DNS for `api.stellarindex.io`
+ `status.stellarindex.io` flipped; rate-limit middleware reads
...
`/etc/stellarindex.toml` on r1 (minimum: USDC, USDT, EURC, AQUA,
...
both fields in r1 config. Restart `stellarindex-api`. Re-curl
...
| ID | Item | Origin |
|---|---|---|
| J1 | [OPERATOR] CF Pages project rename: `stellarindex-showcase` → `stellarindex-explorer` | UI cleanup; aliases work today |
