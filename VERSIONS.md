...
| `stellar/rs-stellar-archivist` | `a6a25033dc2dd1783314ff5b009123e6bfc00e7a` | 2026-04-20 | (no tag yet) | Runtime binary — we call it from scripts. Pin SHA since no tag. **This is the commit our verify-archive integration is verified against, not an enforced apt-install pin**: `configs/ansible/roles/archival-node/defaults/main.yml` (`stellar_archivist_version`) defaults to `""`, which — same posture as `stellar_core_version` — means "never upgrade an already-provisioned host, install apt's latest candidate on a fresh one." That candidate can drift ahead of this SHA. To reproduce this exact commit, an operator must pin `stellar_archivist_version` in inventory to an apt version they've confirmed corresponds to it. |
...
rs-stellar-archivist  (pre-tag; verified against SHA a6a25033 — NOT an
                                enforced apt pin, see the caveat in the
                                pinned-snapshots table above)
