...
# Build + runtime image for stellaratlas-migrate.
...
RUN CGO_ENABLED=0 GOOS=linux \
    go build -trimpath -buildvcs=true \
      ...
      -o /out/stellaratlas-migrate \
      ./cmd/stellaratlas-migrate

FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/stellaratlas-migrate /usr/local/bin/stellaratlas-migrate
# F-1227 (codex audit-2026-05-12): the migrate binary defaults
# `-migrations migrations`, so a runtime image that copies only the
# binary cannot apply schema out of the box — `stellaratlas-migrate
...
# already syncs `/usr/local/share/stellaratlas/migrations` and passes
# `-migrations` explicitly; that path keeps working in parallel.
COPY migrations/ /migrations/
WORKDIR /
USER nonroot:nonroot
# no listening port
ENTRYPOINT ["/usr/local/bin/stellaratlas-migrate"]
