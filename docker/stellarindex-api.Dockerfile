...
# Build + runtime image for stellarindex-api.
...
COPY --from=builder /out/stellarindex-api /usr/local/bin/stellarindex-api
USER nonroot:nonroot
EXPOSE 3000
ENTRYPOINT ["/usr/local/bin/stellarindex-api"]
