...
# Build + runtime image for stellaratlas-sla-probe.
...
RUN CGO_ENABLED=0 GOOS=linux \
    go build -trimpath -buildvcs=true \
      ...
      -o /out/stellaratlas-sla-probe \
      ./cmd/stellaratlas-sla-probe

FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/stellaratlas-sla-probe /usr/local/bin/stellaratlas-sla-probe
USER nonroot:nonroot
# no listening port
ENTRYPOINT ["/usr/local/bin/stellaratlas-sla-probe"]
