...
# Build + runtime image for stellaratlas-aggregator.
...
RUN CGO_ENABLED=0 GOOS=linux \
    go build -trimpath -buildvcs=true \
      ...
      -o /out/stellaratlas-aggregator \
      ./cmd/stellaratlas-aggregator

FROM gcr.io/distroless/static-debian12:nonroot
COPY --from=builder /out/stellaratlas-aggregator /usr/local/bin/stellaratlas-aggregator
USER nonroot:nonroot
EXPOSE 9465
ENTRYPOINT ["/usr/local/bin/stellaratlas-aggregator"]
