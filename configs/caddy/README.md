...
- `Caddyfile.api` — fronts `api.stellarindex.io` → `localhost:3000`. Auto-HTTPS via Let's Encrypt's tls-alpn-01 challenge. Used on R1 today.
...
The API needs `trusted_proxy_cidrs = ["127.0.0.1/32"]` in `/etc/stellarindex.toml`'s `[api]` section so it honours Caddy's `X-Forwarded-*` headers (without that, request-logger + rate-limit middleware key on `127.0.0.1` for every request — a real correctness bug for per-client rate limiting). R1 has this set as of 2026-05-05.
...
1. `api.stellarindex.io` A record → `136.243.90.96` (or AAAA for IPv6)
...
Until the A record exists, Caddy retries acquisition every few minutes and logs `NXDOMAIN looking up A for api.stellarindex.io`. The `:443` listener is up and Caddy serves a self-signed fallback cert for testing on the IP directly. Once the DNS lands, the next ACME retry succeeds and the cert is auto-served from then on (Caddy renews automatically at 1/3-of-lifetime).
...
```sh
# Public TLS
curl -sf https://api.stellarindex.io/v1/healthz             # 200
curl -sI https://api.stellarindex.io/v1/healthz | grep -i strict-transport
# Should see: Strict-Transport-Security: max-age=31536000; includeSubDomains

# HTTP redirects to HTTPS
curl -sI http://api.stellarindex.io/v1/healthz | head -1
# Should see: HTTP/1.1 308 Permanent Redirect

# Active health-check working — kill the API and curl should 502
sudo systemctl stop stellarindex-api
sleep 15
curl -sI https://api.stellarindex.io/v1/healthz | head -1   # 502
sudo systemctl start stellarindex-api
```

## Future: showcase

The `Caddyfile.api` ships a commented-out `stellarindex.io` block ready to serve the static Next.js export from `/var/www/showcase` if/when the showcase moves off Cloudflare Pages. Today the showcase is Cloudflare Pages so the block stays inert.
