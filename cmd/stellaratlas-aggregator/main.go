// Binary stellaratlas-aggregator computes VWAP over the ingested
...
func main() {
	...
	if *cfgPath == "" {
		fmt.Fprintln(os.Stderr, "stellaratlas-aggregator: -config is required")
		flag.Usage()
		os.Exit(2)
	}

	if err := run(*cfgPath, *dryRun); err != nil {
		fmt.Fprintf(os.Stderr, "stellaratlas-aggregator: %v\n", err)
		os.Exit(1)
	}
}
...
func run(cfgPath string, dryRun bool) error {
	...
	// Mirrors the indexer's pattern (cmd/stellaratlas-indexer/
	// main.go::startMetricsServer). Aggregator counters
	// (stellaratlas_aggregator_ticks_total, _vwap_writes_total,
	...
	// `stellaratlas_ingest_gap_ledgers_significant` alert. The
...
// lets `stellaratlas_aggregator_supply_refresh_total{asset_key,outcome}`
...
// as cmd/stellaratlas-ops/supply.go::resolveSnapshotLedger but
...
// cmd/stellaratlas-ops/supply.go::supplyStoreLookup.
...
// pattern from cmd/stellaratlas-ops/supply.go::supplyChainReader.
...
// operator tuning. Parallel to cmd/stellaratlas-indexer's
...
// level. Parallel to cmd/stellaratlas-indexer.
...
func mkLogger(cfg config.ObsConfig) *slog.Logger {
	return obs.NewLogger(cfg, "stellaratlas-aggregator")
}
...
// `cmd/stellaratlas-api/main.go::buildDivergenceReferences` —
