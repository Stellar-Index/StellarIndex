// Binary stellaratlas-sla-probe is the executable SLA-evidence
// suite. It drives load against a deployed Stellar Atlas API and
...
func main() {
	// API key default falls through to STELLARATLAS_PROBE_API_KEY so
	// the systemd unit can pass it via Environment= without leaking
	// it onto the command line (visible in ps).
	defaultAPIKey := os.Getenv("STELLARATLAS_PROBE_API_KEY")
	...
	var (
		...
		apiKey       = flag.String("api-key", defaultAPIKey, "API key for Authorization: Bearer header. Defaults to $STELLARATLAS_PROBE_API_KEY. Without one the probe hits the anonymous-tier rate limit (60 req/min) and reads as a fail.")
	...
	if *baseURL == "" {
		fmt.Fprintln(os.Stderr, "stellaratlas-sla-probe: -base-url is required")
		flag.Usage()
		os.Exit(2)
	}
	...
	for _, p := range pairFlag {
		...
		if len(parts) != 2 {
			fmt.Fprintf(os.Stderr, "stellaratlas-sla-probe: invalid -pair %q (want asset,quote)\n", p)
			os.Exit(2)
		}
	...
	if *textfileOut != "" {
		if err := writeTextfileAtomic(*textfileOut, &rep); err != nil {
			fmt.Fprintf(os.Stderr, "stellaratlas-sla-probe: write textfile: %v\n", err)
			os.Exit(2)
		}
	}
	...
	for w := 0; w < concurrency; w++ {
		go func(workerID int) {
			...
			for {
				...
				// what tripped stellaratlas_sla_probe_unit_failed (#54).
				if ctx.Err() != nil {
					return
				}
				mu.Lock()
...
func printText(w io.Writer, rep *report) {
	fmt.Fprintf(w, "stellaratlas-sla-probe — %s\n", rep.BaseURL)
