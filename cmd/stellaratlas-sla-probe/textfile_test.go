...
func TestWriteTextfile_PassRun(t *testing.T) {
	...
	wants := []string{
		`stellaratlas_sla_probe_latency_ms{endpoint="price",quantile="0.95"} 45.000`,
		`stellaratlas_sla_probe_latency_ms{endpoint="healthz",quantile="0.5"} 3.000`,
		`stellaratlas_sla_probe_availability_pct{endpoint="price"} 100.000`,
		`stellaratlas_sla_probe_freshness_sec{endpoint="price"} 1.500`,
		`stellaratlas_sla_probe_samples{endpoint="price"} 100`,
		`stellaratlas_sla_probe_run_duration_seconds 30.000`,
		`stellaratlas_sla_probe_unit_failed 0`,
		`stellaratlas_sla_probe_last_pass_timestamp `, // unix value varies
	}
	for _, w := range wants {
...
func TestWriteTextfile_FailRun(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "stellaratlas_sla_probe_unit_failed 1") {
		t.Errorf("fail run should emit unit_failed=1; got:\n%s", out)
	}
	if strings.Contains(out, "stellaratlas_sla_probe_last_pass_timestamp") {
...
func TestWriteTextfile_OmitsFreshnessBlockWhenAbsent(t *testing.T) {
	...
	if strings.Contains(buf.String(), "stellaratlas_sla_probe_freshness_sec") {
...
func TestWriteTextfileAtomic_RoundTrip(t *testing.T) {
	...
	if !strings.Contains(string(body), "stellaratlas_sla_probe_unit_failed 0") {
