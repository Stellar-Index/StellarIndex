// Binary stellaratlas-migrate applies and rolls back TimescaleDB
...
// Subcommands:
//
//	stellaratlas-migrate up              Apply every pending migration.
//	stellaratlas-migrate down [N]        Roll back last N migrations (default 1).
//	stellaratlas-migrate status          Show current + target version.
//	stellaratlas-migrate version         Build version.
//	stellaratlas-migrate help            Print usage.
//
// DSN resolution order: --dsn flag, then STELLARATLAS_POSTGRES_DSN env,
...
func main() { //nolint:gocognit,gocyclo // dispatch-heavy; splitting would reduce linearity
	fs := flag.NewFlagSet("stellaratlas-migrate", flag.ContinueOnError)
	dsn := fs.String("dsn", "", "Postgres DSN (overrides STELLARATLAS_POSTGRES_DSN env)")
	...
	resolvedDSN := *dsn
	if resolvedDSN == "" {
		resolvedDSN = os.Getenv("STELLARATLAS_POSTGRES_DSN")
	}

	switch args[0] {
	case "up":
		if resolvedDSN == "" {
			die("no DSN: set STELLARATLAS_POSTGRES_DSN or pass -dsn")
		}
...
func die(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "stellaratlas-migrate: "+format+"\n", args...)
	os.Exit(1)
}

func printUsage(fs *flag.FlagSet) {
	fmt.Fprintf(os.Stderr, `stellaratlas-migrate %s

Apply + manage TimescaleDB schema migrations.

Usage:
  stellaratlas-migrate [-dsn DSN] [-migrations DIR] <subcommand> [args]
...
Environment:
  STELLARATLAS_POSTGRES_DSN   Postgres DSN, used when -dsn is not set.
                             Example: postgres://user:pass@host:5432/db?sslmode=disable

Examples:
  export STELLARATLAS_POSTGRES_DSN="postgres://stellaratlas@localhost/stellaratlas?sslmode=disable"
  stellaratlas-migrate up
  stellaratlas-migrate status
  stellaratlas-migrate down 1
`)
}
