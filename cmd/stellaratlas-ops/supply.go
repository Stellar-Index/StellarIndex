...
// `supplyAggregatorLedgers` in cmd/stellaratlas-aggregator/main.go
...
func runSupplyCrossCheck(ctx context.Context, store *timescale.Store, otherRaw, primaryKey string, primarySnap supply.Supply) error {
	...
	if result.WithinTolerance {
		...
		fmt.Println("  next action:          stellaratlas-ops supply audit <asset> -history-hours 24 to identify when divergence appeared")
	}
	fmt.Println()
