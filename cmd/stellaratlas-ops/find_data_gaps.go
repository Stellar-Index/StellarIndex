...
// `stellaratlas-ops backfill --from <gap.start> --to <gap.end>
...
func writeFindDataGapsText(r findDataGapsReport) {
	...
	for i, g := range r.Gaps {
		...
		case "soroban-events":
			_, _ = fmt.Fprintf(os.Stdout,
				"      %2d  stellaratlas-ops backfill --config /etc/stellaratlas.toml --from %d --to %d --source soroban-events\n",
				i+1, g.Start, g.End)
		case "sdex":
			_, _ = fmt.Fprintf(os.Stdout,
				"      %2d  stellaratlas-ops backfill --config /etc/stellaratlas.toml --from %d --to %d --source sdex\n",
				i+1, g.Start, g.End)
		default:
			_, _ = fmt.Fprintf(os.Stdout,
				"      %2d  stellaratlas-ops projector-replay --config /etc/stellaratlas.toml --source %s --from %d\n",
