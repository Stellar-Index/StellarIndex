...
// promLabels renders st's endpoint identity as a braced Prometheus label
// set (braces included so lint-textfile-exposition reads `name%s` as a
// runtime label set), adding a `pair` label only when st.Pair is set —
