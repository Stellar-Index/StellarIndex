// Binary stellaratlas-indexer runs the production ingestion
// pipeline:
//
...
import (
	...
	"github.com/StellarAtlas/stellar-atlas/internal/dispatcher/statsflush"
	...
	"github.com/StellarAtlas/stellar-atlas/internal/projector"
	...
	externalchainlink "github.com/StellarAtlas/stellar-atlas/internal/sources/external/chainlink"
...
func realMain() int {
	...
	if *cfgPath == "" {
		fmt.Fprintln(os.Stderr, "stellaratlas-indexer: -config is required")
		flag.Usage()
		return 2
	}

	if err := run(*cfgPath, *dryRun); err != nil {
		fmt.Fprintf(os.Stderr, "stellaratlas-indexer: %v\n", err)
		return 1
	}
	return 0
}
...
func run(cfgPath string, dryRun bool) error {
	...
	// 60 s and emits `stellaratlas_postgres_ping_total` +
	// `stellaratlas_postgres_ping_failure_streak`. This is the
	...
	// `stellaratlas-ops seed-soroswap-pairs` once to bootstrap.
	...
	// `stellaratlas-ops seed-protocol-contracts -source <name>` once to
	...
	if cfg.Storage.ClickHouseLiveSink {
		...
		// stellaratlas_ch_live_sink_ledgers_total delta on a short interval, so a
...
// 15 s and emits the per-tick delta on stellaratlas_ch_live_sink_ledgers_total
...
// `stellaratlas_postgres_ping_failing` alert instead of hours of
// silent drift.
//
...
// stellaratlas-ops (backfill subcommand) hit the same code path.
...
// every boot. Backfill (`stellaratlas-ops backfill`) does NOT call
...
func mkLogger(cfg config.ObsConfig) *slog.Logger {
	return obs.NewLogger(cfg, "stellaratlas-indexer")
}
