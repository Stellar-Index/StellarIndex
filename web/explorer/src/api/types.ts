...
export interface paths {
    ...
    "/observations": {
         ...
         *     - **`flags.divergence_checked` is likewise always `false` here.**
         *       The cross-reference check vouches for an aggregated VWAP, and
         *       this surface serves raw per-source trades with no aggregated
         *       value to vouch for, so `divergence_warning` is never meaningful
         *       on this endpoint. Use `/v1/price` or `/v1/vwap` for a
         *       cross-referenced number.
    ...
    "/observations/stream": {
         ...
         *     - `flags.stale` and `flags.divergence_checked` are always
         *       `false` on every tick, for the reasons given on the request
         *       endpoint: this is the raw per-source surface, with nothing
         *       aggregated for a staleness contract or a cross-reference
         *       verdict to vouch for.
    ...
    "/history": {
         ...
         *
         *     XLM's canonical spellings (`native`, `crypto:XLM`, its SAC) are
         *     different venue populations, so a pair with no trades under the
         *     requested spelling is retried under the others and the first
         *     populated one answers. Each row reports the `base_asset` /
    ...
    "/history/since-inception": {
         ...
         *     being coarsened. Measured 2026-09-09 on `native`/`fiat:USD`:
         *     `granularity=1m` returned exactly 50 000 points ending
         *     2018-02-21, while `granularity=1d` returned 3 341 points
         *     spanning 2017-01-17 to yesterday. `flags.stale` is raised when
         *     the series may be short. For a bounded window with an
         *     automatically fitted width, use `/v1/chart`.
         *
         *     The base leg is named `asset` here, NOT `base`, and there is no
         *     `base=` alias — unlike `/v1/history`, whose canonical spelling
         *     is `base=` with `asset=` accepted as an alias. The two history
         *     surfaces are deliberately asymmetric because this one is a
         *     price series keyed like `/v1/price`, not a trade feed.
         *
    ...
    "/chart": {
         ...
         *
         *     XLM's canonical spellings (`native`, `crypto:XLM`, its SAC) are
         *     different venue populations, so a pair with no buckets under the
         *     requested spelling is retried under the others and the first
         *     populated series answers. An alias spelling is the same asset,
    ...
    "/ohlc": {
         ...
         *        is via one returns an empty series here even while `/v1/chart`
         *        serves it — measured 2026-09-08 on `USDC-GA5Z…/fiat:USD`, where
         *        `/v1/ohlc` returned `intervals: []` and `/v1/chart` returned 363
         *        daily points with `triangulated: true`. Read `intervals: []` as
         *        "no bars on a direct route", NOT as "this pair has no history";
         *        `/v1/chart` is the surface that answers the second question.
         *
         *        This asymmetry is deliberate, not an omission. A price series
         *        composes through a pivot; an OHLC BAR does not. `high(A/C)` is not
         *        `high(A/B) × high(B/C)` — the two legs reach their extremes at
         *        different instants inside the bucket, so multiplying them invents a
         *        high that never traded, and the same argument applies to `low`, and
         *        to `open`/`close` whenever the legs' first and last prints are not
         *        simultaneous. Honest triangulated bars need per-trade recomputation
         *        across both legs, not bar arithmetic. Anything cheaper silently
         *        reports prices that did not occur, which is worse on this surface
         *        than an empty array.
         *
    ...
    "/auth/sep10/challenge": {
         ...
         *     `[api.sep10] seed_env` / `jwt_secret_env` — on a deployment
         *     that already has Redis, which the replay guard requires.
         *
         *     Note also that a JWT from `/v1/auth/sep10/token` is only
         *     honoured on other routes when the deployment sets
         *     `auth_mode = "sep10"`, which turns OFF `sip_*` API keys. Do
         *     not plan on holding both credentials at once.
         */
...
export interface components {
    schemas: {
         ...
         *       Two paths set it. The aggregator's triangulation pass
         *       writes the implied VWAP into the same Redis key with a
         *       `:provenance` marker, and `/v1/price` reads that marker
         *       and forwards the flag. Separately, a request for a fiat
         ...
         *       settlement rate. See docs/methodology/local-currency-pricing.md.
        ...
        TradeRow: {
             ...
             *
             *     The scale is a property of the ROW'S SOURCE, not of the
             *     asset, and a single page MIXES BOTH. On-chain rows (sdex,
             *     soroswap, aquarius, phoenix, comet) carry the asset's own
             *     scale — 7 for native/classic, the token contract's declared
             *     `decimals()` for Soroban tokens. CEX rows (binance, kraken,
             *     bitstamp, coinbase) carry 8 REGARDLESS of the pair: that is
             *     the external normalisation scale, not anything about the
             *     asset.
             *
             *     Never substitute a constant. The error is
             *     payload-undetectable — `price` is quote/base and therefore
             *     scale-invariant, so nothing in the response looks wrong —
             *     and a reader assuming 7 per-asset mis-scaled real rows in
             *     production once already.
             *
             *     Present on /v1/history; omitted on /v1/observations, whose
