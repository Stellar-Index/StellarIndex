...
export function TransactionsView() {
  ...
  const explicitSeq = /^\d+$/.test(seqRaw.trim())
    ? Number(seqRaw.trim())
    : null;
...
function TxTable({
  ...
  const source =
    seq != null ? asExample(`/v1/ledgers/${seq}/transactions`) : undefined;
  if (isError) {
    return (
      <Panel
        headingLevel={2}
  ...
  if (rows.length === 0) {
    return (
      <Panel
        ...
        bodyClassName="text-sm text-ink-muted"
      >
        Ledger #{seq?.toLocaleString('en-US')} closed no transactions. Try an
        older ledger.
      </Panel>
    );
  }
                ...
                <td className="px-4 py-3">
                  {t.memo_type && t.memo_type !== 'none' ? (
                    ...
                    >
                      {t.memo_type}
                      {t.memo
                        ? `: ${t.memo.length > 18 ? `${t.memo.slice(0, 18)}…` : t.memo}`
                        : ''}
                    </span>
                  ) : (
