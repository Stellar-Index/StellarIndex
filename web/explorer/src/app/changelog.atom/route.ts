// /changelog.atom — RFC-4287 syndication feed of Stellar Index
...
//   curl -s https://stellarindex.io/changelog.atom | head -20
...
function renderEntry(r: Release): string {
  const id = `urn:stellarindex:release:${versionSlug(r.version)}`;
  const title = `Stellar Index ${r.version}`;
