...
export function NetworkView() {
  ...
  const statsQ = useQuery<NetworkStats>({
    ...
    queryFn: async () =>
      (await apiGet<Envelope<NetworkStats>>('/v1/network/stats', {})).data,
    staleTime: 30_000,
  ...
  const ledgersQ = useQuery<LedgersPage>({
    ...
    queryFn: async () =>
      (await apiGet<Envelope<LedgersPage>>('/v1/ledgers', { limit: 12 })).data,
    staleTime: 10_000,
  });
  ...
  const upgradeMarkers = completeBuckets.flatMap((b, i) => {
    ...
    if (prev == null || cur == null || prev <= 0 || cur <= 0 || cur === prev)
      return [];
    return [
      {
        time: Math.floor(Date.parse(`${b.day ?? ''}T00:00:00Z`) / 1000),
        label: `protocol v${cur}`,
      },
    ];
  });
        ...
        >
          {[
            ...
            <Link
              key={l.href}
              href={l.href}
              className="border-line bg-surface text-ink-body hover:border-brand-500 hover:text-brand-600 rounded-lg border px-3 py-1 text-sm font-medium shadow-xs"
            >
              {l.label} →
            </Link>
          ))}
            ...
            <LineChart
              ...
              ariaLabel={`Daily ${metric} on the Stellar network over the last ${windowDays} days${
                upgradeMarkers.length > 0
                  ? `, with ${upgradeMarkers.length} protocol upgrade marker(s)`
                  : ''
              }`}
            />
            {upgradeMarkers.length > 0 && (
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      >
        {!served && (
          <p className="text-ink-muted text-sm">
            Chain-state series unavailable — the API isn&apos;t serving fee-pool
            buckets yet.
          </p>
        )}
        {served && feeBurn.length === 0 && (
          <p className="text-ink-muted text-sm">
            Not enough complete days in this window to compute a delta.
          </p>
        )}
        {feeBurn.length > 0 && (
          <LineChart
            ...
            legend={{
              ...
              formatValue: (n) =>
                `${n.toLocaleString('en-US', { maximumFractionDigits: 0 })} XLM`,
            }}
      ...
      >
        {!served && (
          <p className="text-ink-muted text-sm">
            Chain-state series unavailable — the API isn&apos;t serving
            total-coins buckets yet.
          </p>
        )}
        {served && totalCoins.length === 0 && (
          <p className="text-ink-muted text-sm">
            No complete days in this window.
          </p>
        )}
        {totalCoins.length > 0 && (
          <LineChart
...
function HeroStats({ stats: s, tip }: { stats?: NetworkStats; tip?: Ledger }) {
  ...
  const stellarMarkets = onChain.reduce(
    (sum, x) => sum + (x.markets_count_24h ?? 0),
    0,
  );
    ...
    <StatGrid cols={4}>
      ...
      <StatCell>
        <Stat
          ...
          value={s ? formatCompact(s.assets_indexed) : '—'}
        />
      </StatCell>
      <StatCell>
        {totalSupply ? (
    ...
    >
      ...
      {error && (
        <p className="text-ink-muted px-4 pb-4 text-sm">
          Ledgers are unavailable right now.
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      {!isLoading && !isError && rows.length === 0 && (
        <div className="px-4 pb-4">
          <EmptyState title="No Stellar markets returned." />
        </div>
      )}
      {rows.length > 0 && (
        <TableWrap className="rounded-none border-0">
...
function ActiveSources() {
  ...
  return (
    ...
    >
      ...
      {!isLoading && !isError && rows.length === 0 && (
        <div className="px-4 pb-4">
          <EmptyState title="No sources reporting volume." />
        </div>
      )}
      {rows.length > 0 && (
        <TableWrap className="rounded-none border-0">
...
const DEEPER: { href: string; title: string; blurb: string }[] = [
  {
    href: '/operations',
    title: 'Operations',
    blurb: 'Every decoded op, newest first',
  },
  {
    href: '/transactions',
    title: 'Transactions',
    blurb: 'Recent transactions across the chain',
  },
  {
    href: '/ledgers',
    title: 'Ledgers',
    blurb: 'The full ledger chain to genesis',
  },
  {
    href: '/accounts',
    title: 'Accounts',
    blurb: 'Account directory + activity',
  },
  {
    href: '/contracts',
    title: 'Contracts',
    blurb: 'Soroban contracts + their events',
  },
  {
    href: '/protocols',
    title: 'Protocols',
    blurb: 'Per-protocol verified coverage',
  },
  { href: '/dexes', title: 'DEXes', blurb: 'On-chain pools + AMMs' },
  {
    href: '/lending',
    title: 'Lending',
    blurb: 'Blend pools — TVL, utilization, APY',
  },
  {
    href: '/oracles',
    title: 'Oracles',
    blurb: 'Reflector / Band / Redstone feeds',
  },
  { href: '/mev', title: 'MEV', blurb: 'Detected arbitrage cycles' },
  {
    href: '/anomalies',
    title: 'Anomalies',
    blurb: 'Price-freeze + outlier events',
  },
  {
    href: '/divergences',
    title: 'Divergences',
    blurb: 'Cross-checks vs reference venues',
  },
];
    ...
    >
      {cards.map((d) => (
        <Link
          key={d.href}
          href={d.href}
          className="group rounded-card border-line bg-surface hover:border-brand-500 hover:bg-surface-muted border p-3 transition-colors"
        >
          <div className="text-ink group-hover:text-brand-600 text-sm font-medium">
            {d.title}
          </div>
          <div className="text-ink-muted mt-0.5 text-xs">{d.blurb}</div>
        </Link>
      ))}
    </Panel>
  );
