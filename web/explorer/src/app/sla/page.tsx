  ...
  return (
    <div className="mx-auto max-w-4xl space-y-10 px-6 py-10">
      ...
      >
        <div className="overflow-x-auto">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-line divide-y">
              <tr>
                <td className="text-brand-600 py-2 pr-4 font-mono text-xs">
                  p95 latency
                </td>
              ...
              <tr>
                ...
                <td className="py-2">
                  Share of probe requests answered 2xx, sampled per probe run on
                  the API host
                </td>
              </tr>
              <tr>
        ...
        <Aside>
          Freshness is the target most services quietly omit. A price endpoint
          that answers in 5 ms with a four-hour-old number is fast and useless,
          so staleness is bound explicitly rather than left to the latency
          figure to imply. The 30-second bound binds{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            /v1/price/tip
          </code>
          .{' '}
          ...
          deliberately serves the last <em>closed</em> one-minute bucket so that
          every replay and every region returns byte-identical values, which
          makes its{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            observed_at
          </code>{' '}
          30&ndash;150 s old by construction; it is held to that structural
          150-second bound instead, and a breach of it means the closed-bucket
          pipeline has fallen behind its design. The one answer outside that
          bound is a{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            price_type
          </code>{' '}
          ...
          : an operator-declared 1:1 constant served only when no market has
          priced the pair, whose{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            observed_at
          </code>{' '}
          is the declaration&apos;s adoption stamp, not an observation age. Read{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            /v1/price/tip
          </code>{' '}
      ...
      >
        <p>
          A 99.9 % monthly availability objective is an allowance of roughly{' '}
          <strong>43 minutes 12 seconds</strong> of unavailability in a 30-day
          month. That is the budget. It is spent by incidents, by planned
          maintenance that takes the API down, and by failed deploys alike — the
          budget does not distinguish between causes, because your integration
          does not either.
        </p>
        <DefList
          rows={[
      ...
      >
        <p>
          A dedicated probe binary samples the public endpoints on a schedule
          and records per-endpoint p50/p95/p99 latency, availability, and — for
          the price endpoints — freshness computed from the response&apos;s own{' '}
          <code className="bg-surface-subtle rounded-sm px-1 py-0.5 text-xs">
            observed_at
          </code>{' '}
          timestamp. It writes results as metrics, so a target breach is visible
          as a number rather than discovered from a support ticket.
        </p>
        <p>
          The probe is a separate process from the API, but it runs{' '}
          <strong>on the API host</strong> and calls the service&apos;s own
          listener directly. Its latency figures are therefore application time,
          isolated from network, TLS and reverse-proxy time — which is what
          makes them useful for spotting a regression in our code, and what
          makes them <em>lower</em> than the number you will measure from your
          own client.
        </p>
        <p>
          The honest consequence: this probe{' '}
          <strong>does not measure edge availability</strong>. It cannot detect
          a reverse-proxy failure, an expired certificate, a DNS fault or a
          datacentre-network outage — most of what an availability objective
          exists to catch. Those are covered today only by an external
          dead-man&apos;s-switch that alerts when the host stops reporting,
          which detects a hard outage but does not produce an availability
          percentage. No monthly availability figure is published yet, and we
          will not publish one computed this way. An independent off-host probe
          is the work that makes the availability row above a measurement rather
          than an objective; until it ships, treat the latency numbers as
          application-level and the availability target as a commitment we have
          not yet instrumented end-to-end.
        </p>
      </Section>
      ...
      >
        ...
        />
        <Aside>
          Coverage and completeness are a separate, stronger guarantee than
          availability, and they are reported independently — see{' '}
      ...
      >
        <p>
          ...
          carries current availability and any open incident. Past incidents are
          published with their timeline and root cause rather than summarised
          away.
        </p>
      </Section>
    </div>
