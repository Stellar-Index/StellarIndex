...
//
// This is the SAME token set as the explorer (web/explorer/tailwind.config.ts)
// so the status page (status.stellarindex.io) reads as one product with the
// main site. Light mode only. Modern, minimal, tech-forward: an Inter type
// system on a near-white canvas, hairline borders over heavy shadows,
// generous whitespace, and one confident blue accent. The full token set is
// documented in docs/architecture/design-system.md.
//
// Dark mode is intentionally OFF — there is no `dark:` variant in the status
// app and the `dark` class is never applied.
// ─────────────────────────────────────────────────────────────────────────
const config: Config = {
